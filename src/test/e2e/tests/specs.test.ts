...
const liveCaseCourtSearchControllerPage =
  new LiveCaseCourtSearchControllerPage();
...
describe("Unverified user", () => {
  it("should open main page with 'See publications and information from a court or tribunal' title", async () => {
    await homePage.open("");
    expect(await homePage.getPageTitle()).toEqual(
      "Court and tribunal hearings"
    );
  });

  it("should click accept cookies", async () => {
    expect(await homePage.cookieHeader()).toEqual(
  ...
  it("should click on the continue and navigate to View Options page", async () => {
    ...
    expect(await viewOptionPage.getPageTitle()).toEqual(
  ...
  describe("find a court or tribunal", async () => {
    it("should select 'Court or Tribunal hearing Publications' option and navigate to search option page", async () => {
      ...
      expect(await searchPage.getPageTitle()).toEqual(
    ...
    describe("following the 'Select from an A-Z list of courts and tribunals' path", async () => {
      ...
      it("should click on 'Select from an A-Z list of courts and tribunals' link ", async () => {
        ...
        expect(await alphabeticalSearchPage.getPageTitle()).toEqual(
      ...
      it("should select Country Court jurisdiction and Wales region filters", async () => {
        ...
        expect(
          await alphabeticalSearchPage.checkIfSelected("JurisdictionFilter1")
        ).toBeTruthy();
        expect(
          await alphabeticalSearchPage.checkIfSelected("RegionFilter1")
        ).toBeTruthy();
      });

      it("should click on the apply filters button", async () => {
        alphabeticalSearchPage =
      ...
      it("selecting first result should take you to to the summary of publications page", async () => {
        summaryOfPublicationsPage =
          await alphabeticalSearchPage.selectFirstListResult();
        expect(await summaryOfPublicationsPage.getPageTitle()).toEqual(
          "What do you want to view from " + searchTerm + "?"
        );
      });
    ...
    if (process.env.EXCLUDE_E2E === "true") {
      ...
      describe("find live case status updates", async () => {
        ...
        before(async () => {
          await liveCaseCourtSearchControllerPage.open(
        ...
        it("selecting first result should take you to to the live hearings list page", async () => {
          liveCaseStatusPage =
            await liveCaseCourtSearchControllerPage.selectFirstValidListResult();
          expect(await liveCaseStatusPage.getPageTitle()).toContain(
            "Live hearing updates"
          );
        });

        it(`should have '${validCourtName}' as a sub title`, async () => {
          expect(await liveCaseStatusPage.getCourtTitle()).toContain(
            validCourtName
          );
        });
    ...
    describe("find single justice procedure cases", () => {
      ...
      it("should select 'Single Justice Procedure case' option and navigate to Single Justice Procedure case page", async () => {
        ...
        singleJusticeProcedurePage =
          await viewOptionPage.clickContinueSingleJusticeProcedure();
      ...
      it("should select first list item", async () => {
        sjpPublicListPage =
          await singleJusticeProcedurePage.clickSjpPublicListItem();
    ...
    describe("sorting of list table", () => {
      ...
      it("should select the publication with text", async () => {
        courtListPage = await summaryOfPublicationsPage.clickSelectedListItem(
          "Primary Health"
        );
      ...
      it("should sort the table on ascending order", async () => {
        ...
        expect(await courtListPage.getFirstTableRowFirstCell()).toEqual(
          "10 May"
        );
        expect(await courtListPage.getLastTableRowFirstCell()).toEqual(
          "04 October"
        );
      });
  ...
  describe("banner navigation", () => {
    ...
    it("should click on the SJP cases navigation link and take user to SJP list page", async () => {
      ...
      expect(await summaryOfPublicationsPage.getPageTitle()).toContain(
    ...
    it("should click on the Sign in navigation link and take user to the sign in page", async () => {
      ...
      expect(await signInPage.getPageTitle()).toEqual(
  ...
  describe("request an account", () => {
    ...
    it("should click on the create account link", async () => {
      ...
      expect(await createMediaAccountPage.getPageTitle()).toEqual(
    ...
    it("should complete form and continue to confirmation page", async () => {
      ...
      mediaAccountRequestSubmittedPage =
        await createMediaAccountPage.clickContinue();
      expect(await mediaAccountRequestSubmittedPage.getPanelTitle()).toEqual(
        "Details submitted"
      );
    });
  });
...
describe("CFT IDAM user login", () => {
  describe("Sign in using a valid account", () => {
    ...
    it("should select 'With a MyHMCTS account' option, navigate to the login page, and sign in", async () => {
      ...
      await signInPage.enterText(
        process.env.CFT_VALID_USERNAME,
        "CftEmailField"
      );
      await signInPage.enterText(
        process.env.CFT_VALID_PASSWORD,
        "CftPasswordField"
      );
    ...
    it("should sign out and open session logged out page", async () => {
      ...
      expect(await sessionLoggedOutPage.getPageTitle()).toEqual(
  ...
  describe("Sign in using an invalid account", () => {
    ...
    it("should select 'With a MyHMCTS account' option, navigate to the login page, and sign in", async () => {
      ...
      await signInPage.clickContinueForRadio1();
      await signInPage.enterText(
        process.env.CFT_INVALID_USERNAME,
        "CftEmailField"
      );
      await signInPage.enterText(
        process.env.CFT_INVALID_PASSWORD,
        "CftPasswordField"
      );
      cftAuthenticationFailedPage =
        await signInPage.clickSignInCftUnsuccessful();
    });
...
describe("Verified user", () => {
  ...
  describe("add subscription", async () => {
    it("should click on Email Subscriptions and navigate to subscription management page", async () => {
      subscriptionManagementPage =
        await accountHomePage.clickSubscriptionsCard();
      expect(await subscriptionManagementPage.getPageTitle()).toBe(
    ...
    it("should navigate to add subscription page on button click", async () => {
      subscriptionAddPage =
        await subscriptionManagementPage.clickAddNewSubscriptionButton();
      expect(await subscriptionAddPage.getPageTitle()).toBe(
        "How do you want to add an email subscription?"
      );
    });
    ...
    describe("following the URN path", async () => {
      ...
      it("should select 'By unique reference number' option and navigate to search urn page", async () => {
        await subscriptionAddPage.selectOption(
          "SubscriptionAddByUniqueRefNumber"
        );
        subscriptionUrnSearchPage =
          await subscriptionAddPage.clickContinueForUrnSearch();
      ...
      it("should enter text and click continue", async () => {
        ...
        subscriptionUrnSearchResultsPage =
          await subscriptionUrnSearchPage.clickContinue();
        expect(await subscriptionUrnSearchResultsPage.getPageTitle()).toEqual(
          "Search result"
        );
      });
      ...
      it("should click continue to create subscription", async () => {
        pendingSubscriptionsPage =
          await subscriptionUrnSearchResultsPage.clickContinue();
        expect(await pendingSubscriptionsPage.getPageTitle()).toEqual(
    ...
    describe('following the case name path', async () => {
      ...
      it("should open case name search path", async () => {
        ...
        caseNameSearchPage =
          await subscriptionAddPage.clickContinueForCaseName();
        expect(await caseNameSearchPage.getPageTitle()).toBe(
      ...
      it("should search for a valid case name and navigate to results page", async () => {
        ...
        expect(await caseNameSearchResultsPage.getPageTitle()).toBe(
          "Search result"
        );
      });
      ...
      it("should click continue to create subscription", async () => {
        pendingSubscriptionsPage =
          await caseNameSearchResultsPage.clickContinue();
        expect(await pendingSubscriptionsPage.getPageTitle()).toEqual(
          "Confirm your email subscriptions"
        );
      });
    ...
    describe("following court or tribunal path", async () => {
      ...
      it("should open court or tribunal name search page", async () => {
        await subscriptionAddPage.selectOption(
          "SubscriptionAddByCourtOrTribunal"
        );
        locationNameSearchPage =
          await subscriptionAddPage.clickContinueForCourtOrTribunal();
        expect(await locationNameSearchPage.getPageTitle()).toBe(
      ...
      it("should click on the apply filters button", async () => {
        locationNameSearchPage =
          await locationNameSearchPage.clickApplyFiltersButton();
        expect(await locationNameSearchPage.getPageTitle()).toBe(
          "Subscribe by court or tribunal name"
        );
      });
    ...
    describe('following the case reference number path', () => {
      ...
      it("should select 'By case reference number' option and navigate to search case number page", async () => {
        await subscriptionAddPage.selectOption(
          "SubscriptionAddByCaseRefNumber"
        );
        caseReferenceNumberSearchPage =
          await subscriptionAddPage.clickContinueForCaseReferenceNumberSearch();
      ...
      it("should enter text and click continue", async () => {
        ...
        caseReferenceNumberSearchResultPage =
          await caseReferenceNumberSearchPage.clickContinue();
        expect(
          await caseReferenceNumberSearchResultPage.getPageTitle()
        ).toEqual("Search result");
      });

      it("should click continue to create subscription", async () => {
        pendingSubscriptionsPage =
          await caseReferenceNumberSearchResultPage.clickContinue();
        expect(await pendingSubscriptionsPage.getPageTitle()).toEqual(
          "Confirm your email subscriptions"
        );
      });
    ...
    describe("add subscription", async () => {
      ...
      it("should subscribe", async () => {
        subscriptionConfirmedPage =
          await pendingSubscriptionsPage.clickContinue();
        expect(await subscriptionConfirmedPage.getPanelTitle()).toEqual(
    ...
    describe("remove subscription", async () => {
      ...
      it("should click on the first unsubscribe record", async () => {
        deleteSubscriptionPage =
          await subscriptionManagementPage.clickUnsubscribeFromFirstRecord();
      ...
      it("should select yes option and unsubscribe", async () => {
        ...
        unsubscribeConfirmationPage =
          await deleteSubscriptionPage.clickContinueForYes();
        expect(await unsubscribeConfirmationPage.getPanelTitle()).toEqual(
    ...
    describe("bulk unsubscribe", async () => {
      ...
      it("should navigate to bulk unsubscribe page on button click", async () => {
        bulkDeleteSubscriptionsPage =
          await subscriptionManagementPage.clickBulkUnsubscribeButton();
        expect(await bulkDeleteSubscriptionsPage.getPageTitle()).toBe(
          "Bulk unsubscribe"
        );
      });

      it("should select first court subscription", async () => {
        await bulkDeleteSubscriptionsPage.selectOption(
          "CourtSubscriptionCheckbox1"
        );
        expect(
          await bulkDeleteSubscriptionsPage.courtSubscriptionChecked()
        ).toBeTruthy();
      });
      ...
      it("should click on the bulk unsubscribe button", async () => {
        ...
        expect(
          await bulkDeleteSubscriptionsConfirmationPage.getPageTitle()
        ).toBe("Are you sure you want to remove these subscriptions?");
      });

      it("should select yes option to delete the subscription", async () => {
        await bulkDeleteSubscriptionsConfirmationPage.selectOption(
          "BulkUnsubscribeRadioYes"
        );
        bulkDeleteSubscriptionsConfirmedPage =
          await bulkDeleteSubscriptionsConfirmationPage.clickContinueForYes();
        expect(
          await bulkDeleteSubscriptionsConfirmedPage.getPanelTitle()
        ).toEqual("Subscription(s) removed");
      });
    });
  });
  ...
  describe("SJP list download navigation", () => {
    ...
    it("should navigate to list download disclaimer page on download button click", async () => {
      listDownloadDisclaimerPage =
        await sjpPublicListPage.clickDownloadACopyButton();
      expect(await listDownloadDisclaimerPage.getPageTitle()).toBe(
    ...
    it("should agree to the terms and conditions and continue", async () => {
      ...
      expect(await listDownloadFilesPage.getPageTitle()).toEqual(
        "Download your file"
      );
    });
  });
  ...
  describe("banner navigation", () => {
    ...
    it("should click on the Email subscriptions navigation link and take user to subscription management page", async () => {
      subscriptionManagementPage =
        await accountHomePage.clickEmailSubscriptionsNavLink();
      expect(await subscriptionManagementPage.getPageTitle()).toEqual(
        "Your email subscriptions"
      );
    });
    ...
    it("should click on the Home navigation link and take user to your account page", async () => {
      accountHomePage =
        await summaryOfPublicationsPage.clickSignedInHomeBannerLink();
...
describe("Admin level journeys", () => {
  ...
  describe("Manual Upload", () => {
    ...
    it("should complete form and open summary page", async () => {
      ...
      expect(await manualUploadSummaryPage.getPageTitle()).toEqual(
        "Check upload details"
      );
    });
    it("should open upload confirmation page", async () => {
      fileUploadConfirmationPage =
        await manualUploadSummaryPage.clickContinue();
  ...
  describe("Create new account", () => {
    ...
    it("should click on the create new account card", async () => {
      ...
      expect(await createAdminAccountPage.getPageTitle()).toEqual(
        "Create admin account"
      );
    });
    it("should complete form and open summary page", async () => {
      await createAdminAccountPage.completeForm();
      createAdminAccountSummaryPage =
        await createAdminAccountPage.clickContinue();
      expect(await createAdminAccountSummaryPage.getPageTitle()).toEqual(
        "Check account details"
      );
    });
    it("should click confirm and create user account", async () => {
      createAdminAccountSummaryPage =
        await createAdminAccountSummaryPage.clickConfirm();
      expect(await createAdminAccountSummaryPage.getPanelTitle()).toEqual(
  ...
  describe("Manual Removal", () => {
    it("should open remove publication search page", async () => {
      ...
      expect(await searchPublicationPage.getPageTitle()).toEqual(
    ...
    it("should enter valid court in the search field, click continue and open search results page", async () => {
      ...
      searchPublicationResultsPage =
        await searchPublicationPage.clickContinue();
      expect(await searchPublicationResultsPage.getPageTitle()).toEqual(
        "Select content to remove"
      );
    });
    it("should click on the first result and open confirmation page", async () => {
      publicationConfirmationPage =
        await searchPublicationResultsPage.clickRemoveOnFirstRecord();
    ...
    it("should select yes option and remove publication", async () => {
      ...
      removePublicationSuccessPage =
        await publicationConfirmationPage.clickContinueToRemovePublication();
      expect(await removePublicationSuccessPage.getPanelTitle()).toEqual(
        "Success"
      );
    });
  ...
  describe("Manage media account requests journey", () => {
    ...
    it("should start the manage media account request journey", async () => {
      ...
      expect(await mediaAccountRequestsPage.getPageTitle()).toEqual(
    ...
    it("should select view application", async () => {
      mediaAccountReviewPage =
        await mediaAccountRequestsPage.clickViewApplication();
      expect(await mediaAccountReviewPage.getPageTitle()).toEqual(
        "Applicant's details"
      );
    });
    ...
    it("should click approve application", async () => {
      mediaAccountApprovalPage =
        await mediaAccountReviewPage.clickApproveApplication();
    ...
    it("should select reject application", async () => {
      mediaAccountRejectionPage =
        await mediaAccountReviewPage.clickRejectApplication();
    ...
    it("should select yes to reject application", async () => {
      ...
      mediaAccountRejectionConfirmationPage =
        await mediaAccountRejectionPage.clickContinue();
      expect(
        await mediaAccountRejectionConfirmationPage.getPanelTitle()
      ).toEqual("Account has been rejected");
    });
  });
...
describe("System Admin level journeys", () => {
  it("should open Admin Login page", async () => {
    await signInPage.open("/admin-login?p=B2C_1_SignInAdminUserFlow");
    await signInPage.enterText(
      process.env.B2C_SYSTEM_ADMIN_USERNAME,
      "EmailField"
    );
    await signInPage.enterText(
      process.env.B2C_SYSTEM_ADMIN_PASSWORD,
      "PasswordField"
    );
  ...
  describe("Create new system admin account", () => {
    it("should click on the create new account card", async () => {
      createSystemAdminAccountPage =
        await systemAdminDashboard.clickCreateNewAccountCard();
      expect(await createSystemAdminAccountPage.getPageTitle()).toEqual(
    ...
    it("should complete form and open summary page", async () => {
      ...
      createSystemAdminAccountSummaryPage =
        await createSystemAdminAccountPage.clickContinue();
      expect(await createSystemAdminAccountSummaryPage.getPageTitle()).toEqual(
        "Check account details"
      );
    });
  });
  ...
  describe("User management journey", () => {
    ...
    it("should open user management page", async () => {
      ...
      expect(await userManagementPage.getPageTitle()).toEqual(
        "User Management"
      );
    });
    ...
    it("should click the manage link and be taken to the manage user page", async () => {
      ...
      expect(await manageUserPage.getPageTitle()).toEqual(
        "Manage pip-auto-test-admin@hmcts.net"
      );
    });
    ...
    it("should open the update users role confirmation page and click to go back to the dashboard", async () => {
      const updateUserConfirmationPage =
        await updateUserPage.clickContinueButton();
      expect(await updateUserConfirmationPage.getPageTitle()).toEqual(
        "User Updated"
      );
      ...
      const systemAdminDashboardPage =
        await updateUserConfirmationPage.clickDashboardLink();
      expect(await systemAdminDashboardPage.getPageTitle()).toEqual(
        "System Admin Dashboard"
      );
    });
    ...
    it("should open the delete user confirmation page", async () => {
      const deleteUserConfirmationPage =
        await deleteUserPage.clickContinueButton();
      expect(await deleteUserConfirmationPage.getPageTitle()).toEqual(
        "User Deleted"
      );
      expect(await deleteUserConfirmationPage.getPanelBody()).toEqual(
        "All data relating to the user has been deleted," +
  ...
  describe("should open blob view locations page", async () => {
    ...
    it("should load the blob view locations page", async () => {
      blobViewLocationsPage =
        await systemAdminDashboard.clickBlobExplorerLocationsCard();
      expect(await blobViewLocationsPage.getPageTitle()).toEqual(
        "Blob Explorer - Locations"
      );
    });
    it("should choose the first result", async () => {
      blobViewPublicationsPage =
        await blobViewLocationsPage.selectFirstListResult();
      expect(await blobViewPublicationsPage.getPageTitle()).toEqual(
  ...
  describe("should open bulk create media accounts page", async () => {
    ...
    it("should load the bulk create media accounts page", async () => {
      bulkCreateMediaAccountsPage =
        await systemAdminDashboard.clickBulkCreateMediaAccountsCard();
      expect(await bulkCreateMediaAccountsPage.getPageTitle()).toEqual(
    ...
    it("should upload file and open confirmation page", async () => {
      ...
      bulkCreateMediaAccountsConfirmationPage =
        await bulkCreateMediaAccountsPage.clickContinue();
      expect(
        await bulkCreateMediaAccountsConfirmationPage.getPageTitle()
      ).toEqual("Create media accounts confirmation");
    });
  });
