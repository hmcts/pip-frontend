...
const rawDataLive = fs.readFileSync(
  path.resolve(__dirname, "../unit/mocks/liveCaseStatusUpdates.json"),
  "utf-8"
);
...
const rawSJPData = fs.readFileSync(
  path.resolve(__dirname, "../unit/mocks/trimmedSJPCases.json"),
  "utf-8"
);
const rawPublicationData = JSON.parse(
  fs.readFileSync(
    path.resolve(__dirname, "../unit/mocks/SJPMockPage.json"),
    "utf-8"
  )
);
...
sinon
  .stub(LocationRequests.prototype, "getFilteredCourts")
  .returns(allCourtData);
sinon
  .stub(PublicationRequests.prototype, "getPublicationsByCourt")
  .returns(rawPublicationData);
sinon
  .stub(PublicationRequests.prototype, "getIndividualPublicationJson")
  .returns(rawPublicationData);
...
sinon
  .stub(CaseEventGlossaryRequests.prototype, "getCaseEventGlossaryList")
  .returns(caseEventGlossaryData);
...
sinon
  .stub(AccountManagementRequests.prototype, "getPendingMediaApplications")
  .resolves(mediaApplications);
sinon
  .stub(AccountManagementRequests.prototype, "getThirdPartyAccounts")
  .resolves([]);
