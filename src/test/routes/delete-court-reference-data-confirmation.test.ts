...
const courtDeleteStub = sinon.stub(
...
courtStub
  .withArgs("2")
  .resolves({ locationId: 2, jurisdiction: "test", region: "test" });
courtStub
  .withArgs("3")
  .resolves({ locationId: 3, jurisdiction: "test", region: "test" });

courtDeleteStub.withArgs("2").resolves({ isExists: false, errorMessage: "" });
courtDeleteStub
  .withArgs("3")
  .resolves({ isExists: true, errorMessage: "test" });
