...
Scenario('I as an admin user should be able to accept valid media account request', async ({I}) => {
    ...
    I.see('This account will be created and the applicant will be notified to set up their account. If an account' +
...
Scenario('I as an admin user should be able to reject applications and see proper error ' +
    'messages related to media account request', async ({I}) => {
    const testFullName = testConfig.TEST_SUITE_PREFIX + randomData.getRandomString() + ' Surname';
    const emailTestMediaAccount = randomData.getRandomEmailAddress(TEST_WORKER_NUMBER);
    ...
    I.see('You can access the court and tribunal service from the link below should you wish to make a new request.');
    I.seeElement(locate('//a')
        .withAttr({href: 'https://www.gov.uk/guidance/myhmcts-online-case-management-for-legal-professionals'}));
    I.click('#yes');
    I.click('Continue');
    ...
    I.see('Details provided do not match.');
    I.see('What happens next');
    I.see('The applicant [ ' + emailTestMediaAccount + ' ] will now be emailed to notify them why their application ' +
