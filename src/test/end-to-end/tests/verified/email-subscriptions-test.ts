...
import {
    createLocation, uploadPublication
...
Scenario('I as a verified user should be able to subscribe by court name, URN, case id and case name. Also ' +
    'should be able to remove subscription and bulk unsubscribe', async ({I}) => {
    ...
    await uploadPublication('PUBLIC', locationId, displayFrom, displayTo, 'ENGLISH',
        'etDailyList.json', 'ET_DAILY_LIST');
    ...
    I.click(locate('//tr')
        .withText(caseName)
        .find('input')
        .withAttr({id: 'caseSubscription'}));

    I.click(locate('//tr')
        .withText(locationName)
        .find('input')
        .withAttr({id: 'courtSubscription'}));
    I.click(locate('//tr')
        .withText(caseId)
        .find('input')
        .withAttr({id: 'caseSubscription'}));
...
Scenario('I as a verified user should be able to see proper error messages related to email subscriptions',
    async ({I}) => {
    const displayFrom = DateTime.now().toISO({includeOffset: false});
    const displayTo = DateTime.now().plus({days: 1}).toISO({includeOffset: false});
    const [locationId, locationName, locationFileName] = generateTestLocation();
