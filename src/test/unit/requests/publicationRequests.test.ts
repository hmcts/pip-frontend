...
const rawDataPubs = fs.readFileSync(
  path.resolve(
    __dirname,
    "../../../test/unit/mocks/summaryOfPublications.json"
  ),
  "utf-8"
);
...
const rawData = fs.readFileSync(
  path.resolve(__dirname, "../mocks/dailyCauseList.json"),
  "utf-8"
);
...
dataManagementStub
  .withArgs("/publication/locationId/valid")
...
dataManagementStub
  .withArgs("/publication/" + artefactId + "/payload")
  .resolves({ data: dailyCauseListData });
...
dataManagementStub
  .withArgs("/publication/" + artefactId)
  .resolves({ data: metaData });

dataManagementArchiveStub
  .withArgs("/publication/abc1/archive")
  .rejects(errorResponse);
dataManagementArchiveStub
  .withArgs("/publication/abc2/archive")
  .rejects(errorRequest);
dataManagementArchiveStub
  .withArgs("/publication/abc3/archive")
...
describe("getIndividualPubJson()", () => {
  it("should return publication json", async () => {
    expect(
      await publicationRequests.getIndividualPublicationJson(artefactId, userId)
    ).toBe(dailyCauseListData);
  });

  it("should return court PRESTON", async () => {
    return await publicationRequests
      .getIndividualPublicationJson(artefactId, userId)
      .then((data) => {
  ...
  it("should return judge title Mr", async () => {
    return await publicationRequests
      ...
      .then((data) => {
        expect(
          data["courtLists"][0]["courtHouse"]["courtRoom"][0]["session"][0][
            "judiciary"
          ][0]["johTitle"]
        ).toEqual("Mr");
      });
  ...
  it("should return empty list if request fails", async () => {
    expect(
      await publicationRequests.getIndividualPublicationJson("abc1", userId)
    ).toBe(null);
  });

  it("should return empty list if request fails", async () => {
    expect(
      await publicationRequests.getIndividualPublicationJson("abc2", userId)
    ).toBe(null);
  });

  it("should return empty list if message error", async () => {
    expect(
      await publicationRequests.getIndividualPublicationJson("abc3", userId)
    ).toBe(null);
  });
});
...
describe("getIndividualPubMetadata()", () => {
  it("should return publication meta data", async () => {
    expect(
      await publicationRequests.getIndividualPublicationMetadata(
        artefactId,
        userId,
        false
      )
  ...
  it("should return content datetime", async () => {
    return await publicationRequests
      .getIndividualPublicationMetadata(artefactId, userId, false)
      .then((data) => {
...
describe("Get by case value", () => {
  dataManagementStub
    .withArgs("/publication/search/valid/valid")
    .resolves(successResponse);
  dataManagementStub
    .withArgs("/publication/search/valid/invalid")
    .rejects(errorResponse);
  dataManagementStub
    .withArgs("/publication/search/invalid/valid")
    .rejects(errorRequest);
  dataManagementStub
    .withArgs("/publication/search/invalid/invalid")
    .rejects(errorMessage);

  it("should return data on successful get", async () => {
    expect(
      await pubRequests.getPublicationByCaseValue(valid, valid, userId)
    ).toBe(successResponse.data);
  });

  it("should handle error response from returned service returning empty array", async () => {
    expect(
      await pubRequests.getPublicationByCaseValue(valid, invalid, userId)
    ).toStrictEqual([]);
  });

  it("should handle error request from returned service returning empty array", async () => {
    expect(
      await pubRequests.getPublicationByCaseValue(invalid, valid, userId)
    ).toStrictEqual([]);
  });

  it("should handle error request from returned service returning empty array", async () => {
    expect(
      await pubRequests.getPublicationByCaseValue(invalid, invalid, userId)
    ).toStrictEqual([]);
  });
});
...
describe("Get publication by court id", () => {
  dataManagementStub
    ...
    .resolves(successResponse);
  dataManagementStub
    .withArgs("/publication/locationId/invalid")
    .rejects(errorResponse);
  dataManagementStub
    .withArgs("/publication/locationId/test")
    .rejects(errorRequest);
  dataManagementStub
    .withArgs("/publication/locationId/error")
    .rejects(errorMessage);

  it("should return data on successful get", async () => {
    expect(await pubRequests.getPublicationsByCourt(valid, userId, false)).toBe(
  ...
  it("should handle error response from returned service returning empty array", async () => {
    expect(
      await pubRequests.getPublicationsByCourt(invalid, userId, false)
    ).toStrictEqual([]);
  });

  it("should handle error request from returned service returning empty array", async () => {
    expect(
      await pubRequests.getPublicationsByCourt("test", userId, false)
    ).toStrictEqual([]);
  });

  it("should handle error request from returned service returning empty array", async () => {
    expect(
      await pubRequests.getPublicationsByCourt("error", userId, false)
    ).toStrictEqual([]);
  });
});
...
describe("get individual publication metadata", () => {
  it("should return metadata for a given publication", async () => {
    dataManagementStub
      .withArgs("/publication/fakeArtefactId")
      .resolves({ data: pubs });
    const message = await pubRequests.getIndividualPublicationMetadata(
      "fakeArtefactId",
      userId,
      false
    );
  ...
  it("should send an error request to the log if error request exists", async () => {
    dataManagementStub
      .withArgs("/publication/promiseBreakingData")
      .rejects(errorRequest);
    expect(
      await pubRequests.getIndividualPublicationMetadata(
        "promiseBreakingData",
        userId,
        false
      )
  ...
  it("should send an error to the log if error response exists", async () => {
    dataManagementStub
      .withArgs("/publication/brokenPromiseWithErrorResponse")
      .rejects(errorResponse);
  ...
  it("should send an error to the log if error message exists and error request does not exist", async () => {
    dataManagementStub
      .withArgs("/publication/noErrRequest")
      .rejects(errorMessage);
    const message = await pubRequests.getIndividualPublicationMetadata(
      "noErrRequest",
      userId,
...
describe("get count of pubs for each court", () => {
  it("should return the text as a string if successful", async () => {
    dataManagementStub
      .withArgs("/publication/count-by-location")
      .resolves(successResponse);
...
describe("get individual publication file", () => {
  it("should return file for a given publication", async () => {
    ...
    const message = await pubRequests.getIndividualPublicationFile(
      "fakeArtefactId",
      userId
    );
  ...
  it("should send an error request to the log if error request exists", async () => {
    dataManagementStub
      .withArgs("/publication/promiseBreakingData/file")
      .rejects(errorRequest);
    expect(
      await pubRequests.getIndividualPublicationFile(
        "promiseBreakingData",
        userId
      )
    ).toBe(null);
  ...
  it("should send an error to the log if error response exists", async () => {
    dataManagementStub
      .withArgs("/publication/brokenPromiseWithErrorResponse/file")
      .rejects(errorResponse);
    const response = await pubRequests.getIndividualPublicationFile(
      "brokenPromiseWithErrorResponse",
      userId
    );
    expect(response).toBe(null);
...
describe("get individual publication json", () => {
  it("should return json for a given publication", async () => {
    dataManagementStub
      ...
      .resolves(mockJson);
    const message = await pubRequests.getIndividualPublicationJson(
      "fakeArtefactId",
      userId
    );
  ...
  it("should send an error request to the log if error request exists", async () => {
    dataManagementStub
      .withArgs("/publication/promiseBreakingData/payload")
      .rejects(errorRequest);
    expect(
      await pubRequests.getIndividualPublicationJson(
        "promiseBreakingData",
        userId
      )
    ).toBe(null);
  ...
  it("should send an error to the log if error response exists", async () => {
    dataManagementStub
      ...
      .rejects(errorResponse);
    const response = await pubRequests.getIndividualPublicationJson(
      "brokenPromiseWithErrorResponse",
      userId
    );
    expect(response).toBe(null);
...
describe("archive publication", () => {
  it("should return true if valid data is provided", async () => {
    const response = await pubRequests.archivePublication(
      "abc",
      "joe@bloggs.com"
    );
  ...
  it("should handle error response", async () => {
    expect(await pubRequests.archivePublication("abc1", "joe@bloggs.com")).toBe(
      false
    );
  });

  it("should handle error request", async () => {
    expect(await pubRequests.archivePublication("abc2", "joe@bloggs.com")).toBe(
      false
    );
  });

  it("should handle error message", async () => {
    expect(await pubRequests.archivePublication("abc3", "joe@bloggs.com")).toBe(
      false
    );
  });
});
