...
const subscriptionManagementStub = sinon.stub(
  subscriptionManagementApi,
  "post"
);
const subscriptionManagementPutStub = sinon.stub(
  subscriptionManagementApi,
  "put"
);
...
describe(`getUserSubscriptions(${userIdWithSubscriptions}) with valid user id`, () => {
  stub
    .withArgs(`/subscription/user/${userIdWithSubscriptions}`)
    .resolves(subscriptionsData2);

  it("should return user subscription object", async () => {
    const userSubscriptions = await subscriptionActions.getUserSubscriptions(
  ...
  it("should have mocked object in the case subscriptions list", async () => {
    ...
    expect(userSubscriptions.caseSubscriptions[0].caseNumber).toBe(
      mockedCaseSubscription.reference
    );
  });
  ...
  it("should have mocked object in the court subscriptions list", async () => {
    ...
    expect(userSubscriptions.locationSubscriptions[0].locationName).toBe(
      mockedCourtSubscription.name
    );
  });
});
...
describe("getUserSubscriptions error tests", () => {
  beforeEach(() => {
    stub
      .withArgs(`/subscription/user/${userIdWithoutSubscriptions}`)
      .resolves({ data: [] });
...
describe("unsubscribe error states", () => {
  describe("unsubscribe error response", () => {
    deleteStub
      .withArgs(`/subscription/${unsubscribeInvalidData.subscriptionId}`)
  ...
  describe("unsubscribe error request", () => {
    deleteStub
      .withArgs(`/subscription/${errorRequestBodyData.foo}`)
      .rejects(errorRequest);
    it("should return null", async () => {
      const unsubscribe = await subscriptionActions.unsubscribe(
        errorRequestBodyData.foo,
        "2345-2345"
      );
      expect(unsubscribe).toBe(null);
    });
  ...
  describe("unsubscribe error", () => {
    deleteStub
      .withArgs(`/subscription/${errorBodyData.baz}`)
      .rejects({ error: "error" });
    it("should return null", async () => {
      const unsubscribe = await subscriptionActions.unsubscribe(
        errorBodyData.baz,
...
describe("bulkDeleteSubscriptions", () => {
  ...
  it("should return success message if call is successful", async () => {
    deleteStub
      .withArgs("/subscription/bulk")
      .resolves({ data: "unsubscribed successfully" });
    const response = await subscriptionActions.bulkDeleteSubscriptions(
      subscriptions
    );
...
describe("configure list type Location subscriptions for a user", () => {
  it("should return true if call is successful", async () => {
    subscriptionManagementPutStub
      .withArgs("/subscription/configure-list-types")
      .resolves({});
    const subscriptionUpdated =
      await subscriptionActions.configureListTypeForLocationSubscriptions(
        "1",
        {}
      );
  ...
  it("should return false for failure", async () => {
    subscriptionManagementPutStub
      .withArgs("/subscription/configure-list-types/null")
...
describe("retrieve subscription channels", () => {
  it("should return channels if call is successful", async () => {
    stub
      .withArgs("/meta/channels")
      .resolves({ data: ["CHANNEL_A", "CHANNEL_B"] });
