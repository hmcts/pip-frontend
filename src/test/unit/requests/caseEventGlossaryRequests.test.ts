...
describe("getCaseEventGlossaryList()", () => {
  it("should return list of 49 case events status", async () => {
    ...
    return await searchDescriptionRequests
      .getCaseEventGlossaryList()
      .then((data) => {
  ...
  it("should return null if request fails", async () => {
    stub
      .withArgs("/glossary")
      .resolves(Promise.reject({ response: { data: "test error" } }));
    expect(
      await searchDescriptionRequests.getCaseEventGlossaryList()
    ).toHaveLength(0);
  });
