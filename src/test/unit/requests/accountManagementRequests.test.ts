...
describe("Account Management Requests", () => {
  describe("Create Azure Account", () => {
    it("should return true on success", async () => {
      postStub
        .withArgs(azureEndpoint)
        .resolves({ data: { status: "success" } });
      const response = await accountManagementRequests.createAzureAccount(
        mockValidBody,
        mockHeaders
      );
  ...
  describe("Create Media Account", () => {
    ...
    it("should return true on success", async () => {
      ...
      expect(
        await accountManagementRequests.createMediaAccount(mockValidMediaBody)
      ).toBe(true);
    });
  ...
  describe("Bulk Create Media Accounts", () => {
    ...
    it("should return true on success", async () => {
      ...
      expect(
        await accountManagementRequests.bulkCreateMediaAccounts(
          file,
          fileName,
          requester
        )
  ...
  describe("Get media applications", () => {
    const rawData = fs.readFileSync(
      path.resolve(__dirname, "../mocks/mediaApplications.json"),
      "utf-8"
    );
    ...
    it("should return media applications", async () => {
      getStub
        .withArgs("/application/status/PENDING")
        .resolves({ data: mediaApplications });
      expect(
        await accountManagementRequests.getPendingMediaApplications()
      ).toEqual(mediaApplications);
    });
  ...
  describe("Get Media Application By ID", () => {
    ...
    it("should return dummy application on success", async () => {
      getStub
        .withArgs(applicationGetEndpoint + applicationID)
        .resolves({ status: 201, data: dummyApplication });
      const response = await accountManagementRequests.getMediaApplicationById(
        applicationID
      );
  ...
  describe("Get Media Application Image By ID", () => {
    ...
    it("should return dummy application on success", async () => {
      getStub
        .withArgs(imageGetEndpoint + imageID)
        .resolves({ status: 201, data: dummyImage });
      const response =
        await accountManagementRequests.getMediaApplicationImageById(imageID);
  ...
  describe("Update Media Application Status by ID", () => {
    ...
    it("should return dummy application on success", async () => {
      putStub
        ...
        .resolves({ status: 201, data: dummyApplication });
      const response =
        await accountManagementRequests.updateMediaApplicationStatus(
          applicationID,
          status
        );
      expect(response).toBe(dummyApplication);
  ...
  describe("Update Media Account Verification", () => {
    ...
    it("should return confirmation string on success", async () => {
      putStub
        .withArgs(updateAccountEndpoint + oid)
        .resolves({ status: 200, data: "Media Account verified" });
      const response =
        await accountManagementRequests.updateMediaAccountVerification(oid);
  ...
  describe("Update account last signed in date", () => {
    ...
    it("should return confirmation string on success", async () => {
      putStub
        .withArgs(updateAccountEndpoint + oid)
        .resolves({ status: 200, data: "Account updated" });
      const response =
        await accountManagementRequests.updateAccountLastSignedInDate(
          "PI_AAD",
          oid
        );
    ...
    it("should set the correct time", async () => {
      const putStubForDateChecking = putStub.withArgs(
        updateAccountEndpoint + "1234-1234"
      );
      ...
      const lastSignedInDateLuxon = DateTime.fromISO(
        args[1]["lastSignedInDate"],
        { zone: "utc" }
      );
      expect(
        lastSignedInDateLuxon <= DateTime.utc().plus({ minutes: 5 }) &&
  ...
  describe("Get all accounts except third party", () => {
    ...
    it("should return data on success", async () => {
      getStub
        .withArgs(getAllAccountsEndpoint, { params: { pageSize: 25 } })
        .resolves({
          status: 200,
          data: { userId: "321", userProvenance: "userProvenance" },
        });
  ...
  describe("Delete user by user id", () => {
    ...
    it("should return string on deletion success", async () => {
      ...
      const response = await accountManagementRequests.deleteUser(
        idtoUse,
        "1234"
      );
  ...
  describe("Update user by user id", () => {
    ...
    it("should return updated user on success", async () => {
      putStub
        .withArgs(`${updateUserByUserIdEndpoint}${idtoUse}/${role}`)
        .resolves({
          status: 200,
          data: { userId: "321", userProvenance: "userProvenance" },
        });
      const response = await accountManagementRequests.updateUser(
        idtoUse,
        role,
        "1234"
      );
  ...
  describe("Get third party accounts", () => {
    ...
    it("should return third party accounts", async () => {
      ...
      getStub
        .withArgs("/account/all/third-party")
        .resolves({ status: 200, data: thirdPartyAccounts });
  ...
  describe("Get admin by email and provenance", () => {
    ...
    it("should return pi user on success", async () => {
      getStub
        .withArgs(
          `${getAdminUserByEmailAndProvenanceEndpoint}${email}/${provenance}`
        )
        .resolves({
          status: 200,
          data: { userId: "321", userProvenance: "userProvenance" },
      ...
      const response =
        await accountManagementRequests.getAdminUserByEmailAndProvenance(
          email,
          provenance,
          "1234"
        );
  ...
  describe("Create System Admin user", () => {
    ...
    it("should return system admin account", async () => {
      ...
      const response = await accountManagementRequests.createSystemAdminUser(
        systemAdminAccount,
        issuerId
      );
      expect(response).toStrictEqual({
