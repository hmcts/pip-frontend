...
describe('Channel Management requests', () => {
    ...
    describe('File exists', () => {
        it('should return true if file exists', async () => {
            expect(await channelManagementRequests.fileExists('abc'))
                .toEqual(true);
        });

        it('should return false and an error response if get fails', async () => {
            expect(await channelManagementRequests.fileExists('abc1'))
                .toEqual(false);
        });

        it('should return false and an error response if request fails', async () => {
            expect(await channelManagementRequests.fileExists('abc2'))
                .toEqual(false);
        });
    });
    ...
    describe('Get file sizes', () => {
        it('should return true if file exists', async () => {
            expect(await channelManagementRequests.getFileSizes('abc'))
                .toEqual(fileSizeData);
        });

        it('should return false and an error response if get fails', async () => {
            expect(await channelManagementRequests.getFileSizes('abc1'))
                .toBeNull();
        });

        it('should return false and an error response if request fails', async () => {
            expect(await channelManagementRequests.getFileSizes('abc2'))
                .toBeNull();
        });
    });
});
