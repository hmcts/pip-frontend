...
describe('Location get requests', () => {
    beforeEach(() => {
        ...
        stub.withArgs('/locations/filter', {
            params: { regions: regions, jurisdictions: jurisdictions, language: englishLanguage },
        }).resolves({ data: courtList });
        stub.withArgs('/locations/filter', {
            params: { regions: test, jurisdictions: test, language: englishLanguage },
        }).rejects(errorResponse);
        stub.withArgs('/locations/filter', {
            params: { regions: test, jurisdictions: 'error', language: englishLanguage },
        }).rejects(errorMessage);
        stub.withArgs('/locations/filter', {
            params: { regions: test, jurisdictions: 'foo', language: englishLanguage },
        }).rejects(errorRequest);

        stub.withArgs('/locations/filter', {
            params: { regions: welshRegions, jurisdictions: welshJurisdictions, language: welshLanguage },
        }).resolves({ data: courtList });
        stub.withArgs('/locations/filter', {
            params: { regions: test, jurisdictions: test, language: welshLanguage },
        }).rejects(errorResponse);
        stub.withArgs('/locations/filter', {
            params: { regions: test, jurisdictions: 'error', language: welshLanguage },
        }).rejects(errorMessage);
        stub.withArgs('/locations/filter', {
            params: { regions: test, jurisdictions: 'foo', language: welshLanguage },
        }).rejects(errorRequest);

        stub.withArgs('/locations').resolves({ data: courtList });

        courtDeleteStub
            .withArgs('/locations/1', { headers: { 'x-provenance-user-id': adminUserId } })
            .resolves({ data: { isExists: true, errorMessage: 'test' } });
        courtDeleteStub
            .withArgs('/locations/2', { headers: { 'x-provenance-user-id': adminUserId } })
            .rejects(errorResponse);
        courtDeleteStub
            .withArgs('/locations/3', { headers: { 'x-provenance-user-id': adminUserId } })
            .rejects(errorRequest);
        courtDeleteStub
            .withArgs('/locations/4', { headers: { 'x-provenance-user-id': adminUserId } })
            .rejects(errorMessage);
        courtDeleteStub
            .withArgs('/locations/5', { headers: { 'x-provenance-user-id': adminUserId } })
