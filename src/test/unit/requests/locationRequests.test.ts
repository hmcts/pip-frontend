...
describe("Location get requests", () => {
  ...
  it("should return court by name", async () => {
    expect(
      await courtRequests.getLocationByName(courtNameSearch, englishLanguage)
    ).toBe(courtList[0]);
  });

  it("should return null if response fails", async () => {
    expect(await courtRequests.getLocationByName("test", englishLanguage)).toBe(
      null
    );
  });
  ...
  it("should return null if request fails", async () => {
    expect(
      await courtRequests.getLocationByName("testReq", englishLanguage)
    ).toBe(null);
  });

  it("should return null if call fails", async () => {
    expect(
      await courtRequests.getLocationByName("testMes", englishLanguage)
    ).toBe(null);
  });

  it("should return Welsh court by name", async () => {
    expect(
      await courtRequests.getLocationByName(courtWelshNameSearch, welshLanguage)
    ).toBe(courtList[0]);
  });

  it("should return null for Welsh search if response fails", async () => {
    expect(await courtRequests.getLocationByName("test", welshLanguage)).toBe(
      null
    );
  });
  ...
  it("should return nul for Welsh search if request fails", async () => {
    expect(
      await courtRequests.getLocationByName("testReq", welshLanguage)
    ).toBe(null);
  });

  it("should return null for Welsh search if call fails", async () => {
    expect(
      await courtRequests.getLocationByName("testMes", welshLanguage)
    ).toBe(null);
  });

  it("should return list of courts based on search filter", async () => {
    expect(
      await courtRequests.getFilteredCourts(
        regions,
        jurisdictions,
        englishLanguage
      )
  ...
  it("should return null if request fails", async () => {
    expect(
      await courtRequests.getFilteredCourts(test, test, englishLanguage)
    ).toBe(null);
  });

  it("should return null if response fails", async () => {
    expect(
      await courtRequests.getFilteredCourts(test, "error", englishLanguage)
    ).toBe(null);
  });
  ...
  it("should return null if Welsh request fails", async () => {
    expect(
      await courtRequests.getFilteredCourts(test, test, welshLanguage)
    ).toBe(null);
  });

  it("should return null if Welsh response fails", async () => {
    expect(
      await courtRequests.getFilteredCourts(test, "error", welshLanguage)
    ).toBe(null);
  });
  ...
  it("should return null list of courts for error request", async () => {
    ...
    expect(
      await courtRequests.getFilteredCourts(test, "foo", englishLanguage)
    ).toBe(null);
  });
  ...
  it("should not delete the court if active artefact or subscription exists", async () => {
    expect(await courtRequests.deleteCourt(1, adminUserId)).toStrictEqual(
      deletionResponse
    );
  });
