...
import {
  ...
  isPermittedMedia,
  isPermittedAdmin, isPermittedAccountCreation, isPermittedManualUpload, isPermittedMediaAccount,
}
...
describe('Test checking user roles', () => {
  ...
  it('check that check roles returns true when matched', () => {
    const req = {'user': {'_json': {'extension_UserRole': 'SYSTEM_ADMIN'}}, 'sessionOptions' : {'maxAge': maxAge}};
    expect(checkRoles(req, manualUploadRoles)).to.be.true;
    expect(req.sessionOptions.maxAge).to.equal(14400000);
  });
...
describe('Test Authenticated Admin', () => {
  ...
  it('check redirect to account home if verified role', () => {
    ...
    const req = {'user': {'_json': {'extension_UserRole': 'VERIFIED'}}, 'sessionOptions' : {'maxAge': maxAge}};
    const res = {'redirect': mockRedirectFunction};
    checkAuthenticatedAdmin(req, res, mockRedirectFunction, mediaAccountCreationRoles);
...
describe('Test IsPermittedAccountCreation', () => {
  ...
  it('check returns next function if permitted', () => {
    const mockRedirectFunction = jest.fn(() => 4);
    const req = {'user': {'_json': {'extension_UserRole': 'INTERNAL_SUPER_ADMIN_CTSC'}}, 'sessionOptions' : {'maxAge': maxAge}};
  ...
  it('check redirect to admin-dashboard is called if not matched', () => {
    const mockRedirectFunction = jest.fn((argument) => argument);
    const req = {'user': {'_json': {'extension_UserRole': 'INTERNAL_ADMIN_LOCAL'}}, 'sessionOptions' : {'maxAge': maxAge}};
    const res = {'redirect': mockRedirectFunction};
...
describe('Test IsPermittedMediaAccount', () => {
  ...
  it('check returns next function if permitted', () => {
    const mockRedirectFunction = jest.fn(() => 4);
    const req = {'user': {'_json': {'extension_UserRole': 'INTERNAL_ADMIN_CTSC'}}, 'sessionOptions' : {'maxAge': maxAge}};
  ...
  it('check redirect to admin-dashboard is called if not matched', () => {
    const mockRedirectFunction = jest.fn((argument) => argument);
    const req = {'user': {'_json': {'extension_UserRole': 'INTERNAL_SUPER_ADMIN_LOCAL'}}, 'sessionOptions' : {'maxAge': maxAge}};
    const res = {'redirect': mockRedirectFunction};
