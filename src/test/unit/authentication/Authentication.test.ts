...
describe("Authentication", () => {
  ...
  let expect;
  beforeEach(async () => {
    const authenticationImport = await import(
  ...
  it("Should set up passport correctly for azure authentication", () => {
    ...
    expect(passport._strategies["admin-login"].name).to.eql(
    ...
    expect(passport._strategies["media-verification"].name).to.eql(
      "azuread-openidconnect"
    );
    expect(
      passport._strategies["media-verification"]._options.redirectUrl
    ).to.eql(
  ...
  parameters.forEach((parameter) => {
    it(`Test that deserialising a user returns user object from the PI user table for Azure AAD ${parameter.strategy} authentication`, async () => {
      ...
      await firstDeserializer(
        { oid: "1234", flow: "AAD" },
        serializeMockCallback
      );
  ...
  parameters.forEach((parameter) => {
    it(`Test that the call is read from the env variables when passed in for azure ${parameter.strategy} authentication`, async () => {
      ...
      authentication();
      expect(
        passport._strategies[parameter.strategy]._options.identityMetadata
      ).to.contain("https://localhost:8080");
      expect(passport._strategies[parameter.strategy]._options.clientID).to.eql(
        "2"
      );
      expect(
        passport._strategies[parameter.strategy]._options.clientSecret
      ).to.eq("client_secret");
    });
  });
});
