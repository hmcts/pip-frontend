...
describe('Create System Admin Account Summary page', () => {
  ...
  describe('Render confirm page with duplicate error', () => {
    ...
    it('should display error message', () => {
      ...
      expect(errorTitle.innerHTML).contains('This user already has an account. If the user requires a system admin account, their previous account will need to be deleted first before one can be created.', 'Could not find panel message');
    });
  ...
  describe('Render confirm page with above max system admin error', () => {
    ...
    it('should display error message', () => {
      const errorTitle = htmlRes.getElementsByClassName('govuk-error-summary__body')[0];
      expect(errorTitle.innerHTML).contains('The maximum number of System Admin accounts has been reached.', 'Could not find panel message');
    });
  });
