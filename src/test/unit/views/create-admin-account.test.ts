...
describe("Create Admin Account Page", () => {
  describe("on GET", () => {
    ...
    it("should display first name input field", () => {
      ...
      expect(fullNameLabel.innerHTML).contains(
        "First name",
        "Could not find first name label"
      );
      expect(input.getAttribute("name")).equals(
        "firstName",
        "Could not find firstName input"
      );
    });
    ...
    it("should display last name input field", () => {
      ...
      expect(fullNameLabel.innerHTML).contains(
        "Last name",
        "Could not find last name label"
      );
      expect(input.getAttribute("name")).equals(
        "lastName",
        "Could not find lastName input"
      );
    });
    ...
    it("should display email input", () => {
      ...
      expect(emailLabel.innerHTML).contains(
        "Email address",
        "Could not find email label"
      );
      expect(input.getAttribute("name")).equals(
        "emailAddress",
        "Could not find emailAddress input"
      );
      expect(input.getAttribute("type")).equals(
        "email",
        "Could not correct input type"
      );
    });
    ...
    it("should display 4 radio buttons with valid values and hints", () => {
      ...
      const radioHeader = htmlRes.getElementsByClassName(
        "govuk-fieldset__legend"
      )[0];

      const radiosCount = radioButtons.length;
      expect(radioHeader.innerHTML).contains(
        "User role",
        "Could not find radio header"
      );
      ...
      for (let i = 0; i < radiosCount; i++) {
        ...
        const radioHint =
          htmlRes.getElementsByClassName("govuk-radios__hint")[i];
        expect(radio.innerHTML).contains(
          radioLabels[i],
          "Could not find radio with correct label"
        );
        expect(radioHint.innerHTML).contains(
          radioHints[i],
          "Could not find radio with correct label"
        );
      }
  ...
  describe("on POST", () => {
    ...
    it("should display error summary messages", () => {
      ...
      for (let i = 0; i < errorMessage.length; i++) {
        expect(errorMessage[i].innerHTML).contains(
          errors[i],
          "Could not find error for an radio"
        );
      }
    });
  });
