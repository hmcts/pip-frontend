...
const rawData = fs.readFileSync(
  path.resolve(__dirname, "../mocks/CaseEventGlossary.json"),
  "utf-8"
);
...
describe.skip("Case Event Glossary page", () => {
  ...
  it("should contain letters that link to case events glossary", () => {
    const alphabeticalLetters = htmlRes.getElementsByClassName(
      "govuk-link--no-visited-state"
    );

    expect(alphabeticalLetters[0].innerHTML).contains(
      "A",
      "Alphabetical link is not present"
    );
  ...
  it("should contain the correct headers", () => {
    ...
    expect(tableHeaders[1].innerHTML).contains(
      "Hearing status",
      "Hearing status for the cases"
    );
    expect(tableHeaders[2].innerHTML).contains(
      "Description",
      "Description for case status"
    );
  });
