...
describe("Manage third party subscription confirm", () => {
  ...
  thirdPartySubscriptionUpdateStub
    .withArgs(userId, ["LIST_A", "LIST_B"], "CHANNEL_A")
    .resolves();
  ...
  it("should display summary title", () => {
    const panelHeader = htmlRes.getElementsByClassName(
      summaryHeadingPanelClass
    );
    expect(panelHeader[0].innerHTML).contains(
  ...
  it("should display manage third party text", () => {
    ...
    expect(anchor[0].innerHTML).contains(
      "Manage Third Party Users",
      "Manage Users link text incorrect"
    );
  });

  it("should display manage third party link", () => {
    const paragraph = htmlRes.getElementsByClassName(paragraphClass);
    const anchor = paragraph[0]
      .getElementsByTagName(anchorTag)[0]
      .getAttribute("href");
    expect(anchor).to.equal(
      "/manage-third-party-users",
      "Manage Users link is incorrect"
    );
  });
