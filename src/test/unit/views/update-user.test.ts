...
describe('Update User Page', () => {
  ...
  it('should display heading',  () => {
    ...
    expect(header[0].innerHTML).contains('What role would you like test@email.com to have?',
      'Could not find the header');
  });

  it('should display inset text', () => {
    const insetText = htmlRes.getElementsByClassName(insetTextClass);
    expect(insetText[0].innerHTML).contains('This users current role is CTSC Super Admin',
