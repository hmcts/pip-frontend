...
describe("Delete User Confirmation Page", () => {
  ...
  it("should display panel heading", () => {
    ...
    expect(panelHeading[0].innerHTML).contains(
      "User Deleted",
  ...
  it("should display the dashboard link", () => {
    ...
    expect(link[5].innerHTML).contains(
      ...
      "Could not find the dashboard link"
    );
  });

  it("should display the bullet list points", () => {
    const list = htmlRes
      .getElementsByClassName(listClass)[0]
      .getElementsByTagName("li");
    expect(list.length).to.equal(5);
    expect(list[0].innerHTML).contains(
      "upload reference data",
      "Could not find bullet list"
    );
    expect(list[1].innerHTML).contains(
      "upload bulk media users",
      "Could not find bullet list"
    );
    expect(list[2].innerHTML).contains(
      "extract artefact data",
      "Could not find bullet list"
    );
    expect(list[3].innerHTML).contains(
      "edit third party subscriptions",
      "Could not find bullet list"
    );
    expect(list[4].innerHTML).contains(
      "manage users",
      "Could not find bullet list"
    );
  });
