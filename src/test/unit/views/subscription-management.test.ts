...
const expectedCaseSubsTitleWithNoCaseSubs =
...
const expectedRowDateAdded = DateTime.fromISO(
  "2022-01-14T11:30:12.357299"
...
const expectedUnsubscribeLink =
  "delete-subscription?subscription=252899d6-2b05-43ec-86e0-a438d3854fa8";
...
const userSubscriptionsStub = sinon.stub(
  SubscriptionRequests.prototype,
  "getUserSubscriptions"
);
const locationStub = sinon.stub(LocationService.prototype, "getLocationById");

userSubscriptionsStub
  .withArgs("2")
  .returns({ caseSubscriptions: [], locationSubscriptions: [] });
...
describe("Subscriptions Management Page", () => {
  ...
  it("should display bulk unsubscribe button", () => {
    ...
    expect(button[1].innerHTML).contains(
      expectedBulkUnsubscribeButton,
      "Could not find bulk unsubscribe button"
    );
  });
  ...
  it("should display all subscriptions tab with proper link", () => {
    ...
    expect(subscriptionsTabs[0].innerHTML).contains(
      expectedAllSubsTitle,
      "Could not find all subscriptions tab"
    );
    expect(subscriptionsTabs[0].getAttribute("href")).equal(
      "?all",
      "Tab does not contain proper link"
    );
  });
  ...
  it("should display case subscriptions tab with proper link", () => {
    ...
    expect(subscriptionsTabs[1].innerHTML).contains(
      expectedCaseSubsTitle,
      "Could not find case subscriptions tab"
    );
    expect(subscriptionsTabs[1].getAttribute("href")).equal(
      "?case",
      "Tab does not contain proper link"
    );
  });
  ...
  it("should display court subscriptions tab with proper link", () => {
    ...
    expect(subscriptionsTabs[2].getAttribute("href")).equal(
      "?location",
      "Tab does not contain proper link"
    );
  });
  ...
  it("should have correct columns in the cases table", () => {
    const caseHeaders = htmlRes
      .getElementById("cases-table")
      .getElementsByClassName("govuk-table__header");
    expect(caseHeaders[0].innerHTML).contains(
      caseNameColumn,
      "Case name header is not present"
    );
    expect(caseHeaders[1].innerHTML).contains(
      caseReferenceColumn,
      "Case reference header is not present"
    );
    expect(caseHeaders[2].innerHTML).contains(
      dateAddedColumn,
      "Date added header is not present"
    );
    expect(caseHeaders[3].innerHTML).contains(
      actionsColumn,
      "Actions header is not present"
    );
  });
  ...
  it("should have correct columns in the courts table", () => {
    const courtHeaders = htmlRes
      .getElementById("locations-table")
      .getElementsByClassName("govuk-table__header");
    expect(courtHeaders[0].innerHTML).contains(
      courtNameColumn,
      "Court name header is not present"
    );
    expect(courtHeaders[1].innerHTML).contains(
      dateAddedColumn,
      "Date added header is not present"
    );
  ...
  it("case table should have correct column values", () => {
    ...
    expect(subscriptionCaseRowCells[1].innerHTML).contains(
      expectedRowCaseReference
    );
    expect(subscriptionCaseRowCells[2].innerHTML).contains(
      expectedRowDateAdded
    );
  });
  ...
  it("court table should have correct column values", () => {
    const subscriptionCaseRowCells = htmlRes
      .getElementsByClassName("govuk-table__body")[1]
      .getElementsByClassName("govuk-table__cell");
    expect(subscriptionCaseRowCells[0].innerHTML).contains(
      expectedRowCourtName
    );
    expect(subscriptionCaseRowCells[1].innerHTML).contains(
      expectedRowDateAdded
    );
  ...
  it("court table should be sorted by court name", () => {
    ...
    expect(subscriptionCaseRowCells[3].innerHTML).equal(
...
describe("Subscriptions Management Page with case subscription but without location", () => {
  ...
  it("case table should have correct number of rows", () => {
    const subscriptionsCaseRows = htmlRes
      .getElementsByClassName("govuk-table__body")[0]
      .getElementsByClassName("govuk-table__row");
    expect(subscriptionsCaseRows.length).equal(
      expectedCaseRowsCountWithoutLocation
    );
  });
  ...
  it("court table should have correct number of rows", () => {
    const subscriptionsLocationRows =
      htmlRes.getElementsByClassName("govuk-table__body")[1];
...
describe("Subscriptions Management Page with location subscription but without case", () => {
  ...
  it("court table should have correct number of rows", () => {
    const subscriptionsLocationRows = htmlRes
      .getElementsByClassName("govuk-table__body")[0]
      .getElementsByClassName("govuk-table__row");
    expect(subscriptionsLocationRows.length).equal(
      expectedCourtRowsCountWithoutCaseSubs
    );
  });
  ...
  it("court table should have correct column values", () => {
    const subscriptionLocationRowCells = htmlRes
      .getElementsByClassName("govuk-table__body")[0]
      .getElementsByClassName("govuk-table__cell");
    expect(subscriptionLocationRowCells[0].innerHTML).contains(
      expectedRowCourtName
    );
    expect(subscriptionLocationRowCells[1].innerHTML).contains(
      expectedRowDateAdded
    );
  });

  it("should show the list types to receive button", () => {
    const listTypesToReceiveButton =
      htmlRes.getElementsByClassName("govuk-button");
