...
const listSummary =
  "govuk-body govuk-!-font-weight-bold govuk-!-margin-bottom-1";
...
const rawData = fs.readFileSync(
  path.resolve(__dirname, "../mocks/SJPMockPage.json"),
  "utf-8"
);
...
describe("Single Justice Procedure List page", () => {
  describe("user not signed in", () => {
    ...
    it("should display list date", () => {
      ...
      expect(offenderData[0].innerHTML).contains(
        listText,
        "Could not find the list date information"
      );
    });
    ...
    it("should have offender name", () => {
      const offenderData = htmlRes.getElementsByClassName(
        offenderInformationClass
      );
      expect(offenderData[0].innerHTML).contains(
        offenderName,
    ...
    it("should have offender address", () => {
      ...
      expect(offenderData[3].innerHTML).contains(
        offenderAddress,
        "Could not find the offender address"
      );
    });
    ...
    it("should have prosecutor", () => {
      ...
      expect(offenderData[4].innerHTML).contains(
        prosecutor,
        "Could not find the Prosecutor"
      );
    });

    it("should have reporting restriction section", () => {
      const reportingRestrictionSection = htmlRes.getElementsByClassName(
        reportingRestrictionClass
      );
