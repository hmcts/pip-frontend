...
describe('List Download Disclaimer Page', () => {
  describe('without error', () => {
    ...
    it('should have correct page title', () => {
      ...
      expect(pageTitle).contains('Terms and conditions',
        'Page title does not match');
    });

    it('should display header', () => {
      const heading = htmlRes.getElementsByClassName('govuk-heading-l');
      expect(heading[0].innerHTML).contains('Terms and conditions',
        'Header does not match');
    });

    it('should display the body text', () => {
      const bodyText = htmlRes.getElementsByClassName('govuk-body');
      expect(bodyText[0].innerHTML).contains('As a verified user of the CaTH service you are authorised to download this file containing personal protected data.',
        'Body text does not match');
      expect(bodyText[1].innerHTML).contains('It is your responsibility to ensure you comply with any GDPR and/or reporting restrictions regarding the content of this file.',
        'Body text does not match');
    });

    it('should display checkbox', () => {
      const checkboxLabel = htmlRes.getElementsByClassName('govuk-checkboxes__label');
      expect(checkboxLabel[0].innerHTML).contains('Please tick this box to agree to the above terms and conditions',
  ...
  describe('with error', () => {
    ...
    it('should display error messages in the summary', () => {
      ...
      expect(listItems[0].innerHTML).contains('You must agree to the terms and conditions', 'Could not find error');
    });
  });
});
