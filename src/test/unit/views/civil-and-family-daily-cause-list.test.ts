...
sinon
  ...
  .returns(civilAndFamilyDailyCauseListData);
sinon
...
describe("Civil And Family Daily Cause List page", () => {
  ...
  it("should display the site name for both sections", () => {
    ...
    expect(siteAddress[6].innerHTML).contains(
      "Court B",
  ...
  it("should display the site address line 1 for both sections", () => {
    ...
    expect(siteAddress[1].innerHTML).contains(
      "Address Line 1",
      "Could not find the address line 1 in section 1"
    );
    expect(siteAddress[7].innerHTML).contains(
      "Address Line 1",
      "Could not find the address line 1 in section 2"
    );
  });
  ...
  it("should display the site address line 2 for both sections", () => {
    ...
    expect(siteAddress[2].innerHTML).contains(
      "Address Line 2",
      "Could not find the address line 2 in section 1"
    );
    expect(siteAddress[8].innerHTML).contains(
      "Address Line 2",
      "Could not find the address line 2 in section 2"
    );
  });

  it("should display the site town for both sections", () => {
    const siteAddress = htmlRes.getElementsByClassName(siteAddressClass);
    expect(siteAddress[3].innerHTML).contains(
      "Town A",
      "Could not find the town in section 1"
    );
    expect(siteAddress[9].innerHTML).contains(
      "Town B",
      "Could not find the town in section 2"
    );
  });
  ...
  it("should display the site county for both sections", () => {
    ...
    expect(siteAddress[4].innerHTML).contains(
      "County A",
      "Could not find the county in section 1"
    );
    expect(siteAddress[10].innerHTML).contains(
      "County B",
      "Could not find the county in section 2"
    );
  });

  it("should display the site postcode for both sections", () => {
    const siteAddress = htmlRes.getElementsByClassName(siteAddressClass);
    expect(siteAddress[5].innerHTML).contains(
      "AA1 AA1",
      "Could not find the postcode in section 1"
    );
    expect(siteAddress[11].innerHTML).contains(
      "BB1 BB1",
      "Could not find the postcode in section 2"
    );
  });

  it("should display accordion heading", () => {
    const accordion = htmlRes.getElementsByClassName(accordionClass);
    expect(accordion[0].innerHTML).contains(
      accordionHeading,
