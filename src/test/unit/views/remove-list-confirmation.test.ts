...
const PAGE_URL =
  "/remove-list-confirmation?artefact=18dec6ee-3a30-47bb-9fb3-6a343d6b9efb";
...
sinon
  ...
  .resolves(mockArtefact);
sinon
  .stub(LocationService.prototype, "getLocationById")
  .resolves({ locationId: "5", name: "Mock Court" });

expressRequest["user"] = { roles: "SYSTEM_ADMIN" };

sinon
  .stub(PublicationService.prototype, "removePublication")
  .withArgs("foo")
  .resolves(false);
  ...
  describe("without error", () => {
    ...
    it("should display summary table", () => {
      ...
      for (let i = 0; i < summaryKeys.length; i++) {
        expect(summaryKeys[i].innerHTML).contains(
          keyValues[i],
          "Could not find valid summary key"
        );
        expect(summaryValues[i].innerHTML).contains(
          content[i],
          "Could not find valid summary value"
        );
      }
    });
