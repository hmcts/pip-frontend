...
const rawData = fs.readFileSync(path.resolve(__dirname, '../../mocks/kingsBenchDivisionDailyCauseList.json'), 'utf-8');
...
const PAGE_URL = '/kings-bench-division-daily-cause-list?artefactId=abc';

describe('King’s Bench Division Daily Cause Listpage', () => {
    beforeAll(async () => {
        await request(app)
            .get(PAGE_URL)
            .then(res => {
    ...
    it('should display header', () => {
        ...
        expect(header[0].innerHTML).contains('King’s Bench Division Daily Cause List');
    });
