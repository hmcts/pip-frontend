...
const rawData = fs.readFileSync(path.resolve(__dirname, '../../mocks/patentsCourtChdDailyCauseList.json'), 'utf-8');
...
const PAGE_URL = '/patents-court-chd-daily-cause-list?artefactId=abc';

describe('Patents Court (ChD) Daily Cause List page', () => {
    beforeAll(async () => {
        await request(app)
            .get(PAGE_URL)
            .then(res => {
    ...
    it('should display header', () => {
        ...
        expect(header[0].innerHTML).contains('Patents Court (Chancery Division) Daily Cause List');
    });
    ...
    it('should display data source text', () => {
        const text = htmlRes.getElementsByClassName(bodyText);
        expect(text[18].innerHTML).contains('Data Source: Prov1');
    });
});
