...
const rawData = fs.readFileSync(path.resolve(__dirname, '../../mocks/civilCourtsRcjDailyCauseList.json'), 'utf-8');
...
const PAGE_URL = '/civil-courts-rcj-daily-cause-list?artefactId=abc';

describe('Civil Courts at the RCJ Daily Cause List page', () => {
    beforeAll(async () => {
        await request(app)
            .get(PAGE_URL)
            .then(res => {
    ...
    it('should display header', () => {
        ...
        expect(header[0].innerHTML).contains('Civil Courts at the Royal Courts of Justice Daily Cause List');
    });
