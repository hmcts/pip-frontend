...
const rawData = fs.readFileSync(path.resolve(__dirname, '../../mocks/mayorAndCityCivilDailyCauseList.json'), 'utf-8');
...
const PAGE_URL = '/mayor-and-city-civil-daily-cause-list?artefactId=abc';
const WELSH_PAGE_URL = '/mayor-and-city-civil-daily-cause-list?artefactId=abc&lng=cy';

describe('Mayor & City Civil Daily Cause List page in English', () => {
    beforeAll(async () => {
        await request(app)
            .get(PAGE_URL)
            .then(res => {
...
describe('Mayor & City Civil Daily Cause List page in Welsh', () => {
    beforeAll(async () => {
        await request(app)
            .get(WELSH_PAGE_URL)
            .then(res => {
