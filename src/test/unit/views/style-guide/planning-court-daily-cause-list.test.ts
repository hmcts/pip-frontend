...
const rawData = fs.readFileSync(path.resolve(__dirname, '../../mocks/planningCourtDailyCauseList.json'), 'utf-8');
...
const PAGE_URL = '/planning-court-daily-cause-list?artefactId=abc';

describe('Planning Court Daily Cause List page', () => {
    beforeAll(async () => {
        await request(app)
            .get(PAGE_URL)
            .then(res => {
    ...
    it('should display header', () => {
        ...
        expect(header[0].innerHTML).contains('Planning Court Daily Cause List');
    });
