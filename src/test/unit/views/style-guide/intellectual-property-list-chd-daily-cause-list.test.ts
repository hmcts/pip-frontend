...
const rawData = fs.readFileSync(
    path.resolve(__dirname, '../../mocks/intellectualPropertyListChdDailyCauseList.json'),
    'utf-8'
);
...
const PAGE_URL = '/intellectual-property-list-chd-daily-cause-list?artefactId=abc';

describe('Intellectual Property (ChD) Daily Cause List page', () => {
    beforeAll(async () => {
        await request(app)
            .get(PAGE_URL)
            .then(res => {
    ...
    it('should display header', () => {
        ...
        expect(header[0].innerHTML).contains('Intellectual Property (Chancery Division) Daily Cause List');
    });
