...
const rawData = fs.readFileSync(path.resolve(__dirname, '../../mocks/commercialCourtKbDailyCauseList.json'), 'utf-8');
...
const PAGE_URL = '/commercial-court-kb-daily-cause-list?artefactId=abc';

describe('Commercial Court (King’s Bench Division) Daily Cause List page', () => {
    beforeAll(async () => {
        await request(app)
            .get(PAGE_URL)
            .then(res => {
    ...
    it('should display header', () => {
        ...
        expect(header[0].innerHTML).contains('Commercial Court (King’s Bench Division) Daily Cause List');
    });
    ...
    it('should display important information heading 2', () => {
        const text = htmlRes.getElementsByClassName(bodyText);
        expect(text[12].innerHTML).contains('Remote judgments');
    });
