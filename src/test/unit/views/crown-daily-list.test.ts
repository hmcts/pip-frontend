...
describe('Crown daily List page', () => {
  ...
  it('should display the site name for both sections',  () => {
    const siteAddress = htmlRes.getElementsByClassName(siteAddressClass);
    expect(siteAddress[1].innerHTML).contains('Sitting at Court B', 'Could not find the Court name with Sitting at text');
  });
  ...
  it('should display Defendant Name',  () => {
    const cell = htmlRes.getElementsByClassName('govuk-table__cell');
    expect(cell[2].innerHTML).contains('DEFENDANT');
  });
  ...
  it('should display the to be allocated for unallocated case section',  () => {
    const siteAddress = htmlRes.getElementsByClassName(siteAddressClass);
    expect(siteAddress[4].innerHTML).contains('The following cases may be taken in any court', 'Could not find the unallocated case section');
