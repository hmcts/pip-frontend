...
describe("Reference Data Manual upload page", () => {
  describe("on GET", () => {
    ...
    it("should display the download current reference data link", () => {
      ...
      expect(link[2].innerHTML).to.equal(
        "Download current reference data",
        "Could not find download link"
      );
      expect(link[2].outerHTML).contains(
        'href="/manual-reference-data-download"',
        "href not found"
      );
    });
  });
  ...
  describe("on POST", () => {
    ...
    it("should display file too large error", () => {
      const fileError = htmlRes.getElementById(
        "manual-reference-data-upload-error"
      );
