...
describe('Crown Warned List page', () => {
  ...
  it('should display publication date',  () => {
    ...
    expect(date[0].innerHTML).contains('Last Updated 13 September 2022 at 12:30pm', 'Could not find the publication date');
  });
  ...
  it('should display the list information text',  () => {
    ...
    expect(info[0].innerHTML).contains('*denotes a defendant in custody', 'Could not find the list information text');
  });

  it('should display publishing restriction heading',  () => {
    const restriction = htmlRes.getElementsByClassName(restrictionHeadingClass);
    expect(restriction[0].innerHTML).contains('Restrictions on publishing or writing about these cases',
  ...
  it('should display all hearing types in accordion',  () => {
    ...
    expect(accordion[3].innerHTML).to.equal('For Appeal against Conviction', 'Could not find fourth accordion heading');
