...
const proofOfIdViewLink =
  "/media-account-review/image?imageId=12345&applicantId=1234";
const yesRadio = "Yes";
const noRadio = "No";
...
sinon
  .stub(MediaAccountApplicationService.prototype, "getApplicationByIdAndStatus")
  .resolves(dummyApplication);
...
describe("Media Account Approval Page", () => {
  ...
  it("should display table caption", () => {
    ...
    expect(header[0].innerHTML).contains(
      expectedTableCaption,
      "Could not find the table caption"
    );
  });
  ...
  it("should display email value", () => {
    const value = htmlRes.getElementsByClassName(summaryCell);
    expect(value[1].innerHTML).contains(
      emailValue,
  ...
  it("should display proof of id header", () => {
    const header = htmlRes.getElementsByClassName(summaryHeader);
    expect(header[4].innerHTML).contains(
      proofOfIdHeader,
      "Could not find the proof of id header"
    );
  });

  it("should display proof of id value", () => {
    const value = htmlRes.getElementsByClassName(summaryCell);
    expect(value[4].innerHTML).contains(
      proofOfIdValue,
      "Could not find the proof of id value"
    );
  });
  ...
  it("should display proof of id view text", () => {
    ...
    expect(anchor.innerHTML).contains(
      proofOfIdView,
      "Could not find the proof of id view text"
    );
  });
  ...
  it("should display proof of id view link", () => {
    const value = htmlRes.getElementsByClassName(summaryActions);
    const anchor = value[4].getElementsByTagName("a")[0];
    expect(anchor.getAttribute("href")).contains(
      proofOfIdViewLink,
      "Could not find the proof of id view text"
    );
  });
  ...
  it("should display yes radio button", () => {
    ...
    expect(radioButton[0].innerHTML).contains(
      yesRadio,
      "Could not find the yes radio button"
    );
  });

  it("should display no radio button", () => {
    const radioButton = htmlRes.getElementsByClassName(radioLabel);
    expect(radioButton[1].innerHTML).contains(
      noRadio,
      "Could not find the no radio button"
    );
  });
  ...
  it("should display continue button", () => {
    ...
    expect(button[0].innerHTML).contains(
      continueButtonText,
      "Could not find the continue button text"
    );
  });
});
...
describe("Media Account Approval No Selection", () => {
  ...
  it("should display the error title when no selection is entered", () => {
    const errorSummaryResult =
      htmlRes.getElementsByClassName(errorSummaryClass);
    expect(errorSummaryResult[0].innerHTML).contains(
      errorSummary,
