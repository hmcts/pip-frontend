...
describe('List Download Files Page', () => {
  ...
  it('should display the body text', () => {
    ...
    expect(bodyText[0].innerHTML).contains('Save your file somewhere you can find it. You may need to print it or show it to someone later.',
      'Body text does not match');
    expect(bodyText[3].innerHTML).contains('If you have any questions, call 0300 303 0656.',
      'Body text does not match');
  });

  it('should display the link', () => {
    const bodyText = htmlRes.getElementsByClassName('govuk-body');
    expect(bodyText[1].innerHTML).contains('govuk-link',
      'Could not find link');
    expect(bodyText[1].innerHTML).contains('Download this PDF (1.1MB) to your device',
      'Body text does not match');
    expect(bodyText[2].innerHTML).contains('govuk-link',
      'Could not find link');
    expect(bodyText[2].innerHTML).contains('Download this Microsoft Excel spreadsheet (25.2KB) to your device',
      'Body text does not match');
  });
});
