...
sinon
  .stub(PublicationRequests.prototype, "getPublicationByCaseValue")
  .returns(data);
...
describe("Case name search results page", () => {
  ...
  it("should display results count message", () => {
    ...
    expect(resultsMessage[0].innerHTML).contains(
      "5  result(s) successfully found",
  ...
  it("should contain expected column headings", () => {
    ...
    expect(tableHeaders[0].innerHTML).contains(
      "Select a Result",
      "Could not find select a result header"
    );
    ...
    expect(tableHeaders[2].innerHTML).contains(
      "Reference Number",
      "Could not find case reference number header"
    );
  });
  ...
  it("should display checkboxes", () => {
    const checkBoxes = htmlRes.querySelectorAll(
      ".govuk-table__body .govuk-checkboxes__input"
    );
