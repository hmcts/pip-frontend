...
describe("Subscriptions List Type Confirmed Page", () => {
  ...
  it("should display confirmation panel with correct title and message", () => {
    const panel = htmlRes.getElementsByClassName(
      "govuk-panel--confirmation"
    )[0];
    expect(
      panel.getElementsByClassName("govuk-panel__title")[0].innerHTML
    ).contains(
      "Court Subscription(s) refined",
      "Could not find panel title or is incorrect"
    );
    expect(
      panel.getElementsByClassName("govuk-panel__body")[0].innerHTML
    ).contains(
  ...
  it("should contain you account url", () => {
    const youAccountLink = htmlRes
      .getElementsByClassName("govuk-body")[0]
