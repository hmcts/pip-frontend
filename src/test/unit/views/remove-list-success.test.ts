...
describe('Remove List Success Page', () => {
  ...
  it('should display links to other actions with correct paths', () => {
    ...
    expect(links[3].innerHTML).to.equal('Remove another file');
    expect(links[3].getAttribute('href')).contains('remove-list-search');
    expect(links[4].innerHTML).to.equal('Upload a file');
    expect(links[4].getAttribute('href')).contains('manual-upload');
    expect(links[5].innerHTML).to.equal('Home');
    expect(links[5].getAttribute('href')).contains('admin-dashboard');
  });
});
