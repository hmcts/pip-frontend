...
describe("Bulk Create Media Accounts Confirmed Page", () => {
  ...
  it("should display confirmation within the panel", () => {
    ...
    expect(panelTitle.innerHTML).to.contains(
      "Media accounts created",
      "Success panel title does not match"
    );
  ...
  it("should contain the bulk create media accounts link", () => {
    ...
    expect(link.innerHTML).to.equal(
      "Upload another file",
  ...
  it("should contain the system admin dashboard link", () => {
    const link = htmlRes
      .getElementsByClassName("govuk-body")[2]
      .getElementsByClassName("govuk-link")[0];
