...
describe('Admin Management Page On Error', () => {
  ...
  it('should display minimum input error message', () => {
    const errorSummary = htmlRes.getElementsByClassName(errorSummaryBodyClass);
    expect(errorSummary[0].innerHTML).contains('There is no user matching that email address', 'Could not find error message');
  });
  ...
  it('should display additional message', () => {
    ...
    expect(additionalMessage[0].innerHTML).contains('What is the users email address?', 'Could not find additional message');
  });
