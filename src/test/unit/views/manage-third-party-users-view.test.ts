...
describe('Manage third party users - view', () => {
  ...
  describe('Manage third party users - view without subs', () => {
    ...
    beforeAll(async () => {
      await request(app).get(PAGE_URL + '?userId=' + userId).then(res => {
    ...
    it('manage subscriptions button should be correct', () => {
      ...
      expect(buttons[0].innerHTML).contains('Manage Subscriptions',
  ...
  describe('Manage third party users - with subs', () => {
    ...
    beforeAll(async () => {
      await request(app).get(PAGE_URL + '?userId=' + userIdWithSubs).then(res => {
        htmlRes = new DOMParser().parseFromString(res.text, 'text/html');
        htmlRes.getElementsByTagName('div')[0].remove();
      });
    });
