...
sinon
  .stub(PublicationService.prototype, "getCaseByCaseNumber")
...
describe("Case Reference Search Results Page", () => {
  ...
  it("should display back button", () => {
    ...
    expect(backButton[0].getAttribute("href")).equal(
      "#",
      "Back button does not contain correct link"
    );
  });
  ...
  it("should display second table header", () => {
    ...
    expect(tableHeader2[0].innerHTML).contains(
      "Case name",
  ...
  it("should contain 2 rows including the header row", () => {
    ...
    expect(rows.length).equal(
      2,
      "Table did not contain expected number of rows"
    );
  });
  ...
  it("should contain rows with correct values", () => {
    ...
    expect(items[0].innerHTML).contains(
      "635356",
      "Case reference no does not exist"
    );
    expect(items[1].innerHTML).contains(
      "case name 1",
