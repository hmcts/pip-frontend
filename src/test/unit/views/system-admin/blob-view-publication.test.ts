...
describe('Blob explorer publication page', () => {
    describe('for existed location', () => {
        describe('with JSON publication', () => {
            describe('of public sensitivity', () => {
                beforeAll(async () => {
                    app.request['user'] = { userId: '1', roles: 'SYSTEM_ADMIN' };

                    await request(app)
                        .get('/blob-view-publication?artefactId=1')
                        .then(res => {
            ...
            describe('of private sensitivity', () => {
                beforeAll(async () => {
                    app.request['user'] = { userId: '1', roles: 'SYSTEM_ADMIN' };

                    await request(app)
                        .get('/blob-view-publication?artefactId=2')
                        .then(res => {
                            htmlRes = new DOMParser().parseFromString(res.text, 'text/html');
                        });
                });

                it('should not display the link to the template', () => {
                    const link = htmlRes.getElementsByClassName(linkClass)[5];
                    expect(link.innerHTML).not.contains('Link to rendered template', 'Found the link to the template');
                });
            });

            describe('of classified sensitivity', () => {
                beforeAll(async () => {
                    app.request['user'] = { userId: '1', roles: 'SYSTEM_ADMIN' };

                    await request(app)
                        .get('/blob-view-publication?artefactId=3')
                        .then(res => {
                            htmlRes = new DOMParser().parseFromString(res.text, 'text/html');
                        });
                });
        ...
        describe('with Public flat file publication', () => {
            beforeAll(async () => {
                app.request['user'] = { userId: '1', roles: 'SYSTEM_ADMIN' };

                await request(app)
                    .get('/blob-view-publication?artefactId=4')
                    .then(res => {
                        htmlRes = new DOMParser().parseFromString(res.text, 'text/html');
                    });
            });
        ...
        describe('with Private flat file publication', () => {
            beforeAll(async () => {
                app.request['user'] = { userId: '1', roles: 'SYSTEM_ADMIN' };

                await request(app)
                    .get('/blob-view-publication?artefactId=5')
                    .then(res => {
                        htmlRes = new DOMParser().parseFromString(res.text, 'text/html');
                    });
            });

            it('should not display the link to flat file', () => {
                const link = htmlRes.getElementsByClassName(linkClass)[5];
                expect(link.innerHTML).not.contains('Link to file', 'Found the link to flat file');
            });
        });

        describe('with Classified flat file publication', () => {
            beforeAll(async () => {
                app.request['user'] = { userId: '1', roles: 'SYSTEM_ADMIN' };

                await request(app)
                    .get('/blob-view-publication?artefactId=6')
                    .then(res => {
                        htmlRes = new DOMParser().parseFromString(res.text, 'text/html');
                    });
            });
