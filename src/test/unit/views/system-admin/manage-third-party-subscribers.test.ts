...
describe('Manage third party subscribers', () => {
    ...
    it('should display create new subscriber button', () => {
        ...
        expect(button[0].innerHTML).contains(
            ...
            'Could not find the create new subscriber button'
        );
    });
    ...
    it('should display created date header', () => {
        const columns = htmlRes.getElementsByClassName(columnClass);
        expect(columns[1].innerHTML).contains('Created Date', 'Could not find the header');
    });
    ...
    it('row one should have the correct created date', () => {
        const cells = htmlRes.getElementsByClassName(cellClass);
        expect(cells[1].innerHTML).contains('18th November 2022', 'Created date is incorrect');
    });

    it('row one should have the correct manage text', () => {
        const cells = htmlRes.getElementsByClassName(cellClass);
        expect(cells[2].innerHTML).contains('Manage', 'Manage link does not have the correct text');
    });

    it('row one should have the correct manage link', () => {
        const linkHref = htmlRes.getElementsByClassName(cellClass)[2].getElementsByTagName('a')[0].getAttribute('href');

        expect(linkHref).equals(
            '/manage-third-party-subscribers/view?userId=' + '1234-1234',
