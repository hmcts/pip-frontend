...
const expectedFileQuestion =
  "Manually upload a csv, doc, docx, htm, html, json, or pdf file, max size 2MB";
...
describe("Manual upload page", () => {
  describe("on GET", () => {
    ...
    it("should contain file upload question inset", () => {
      ...
      expect(insetFileUpload[0].innerHTML).contains(
        expectedFileQuestion,
        "Could not find file upload"
      );
      expect(
        insetFileUpload[0].getElementsByTagName("input")[0].getAttribute("type")
      ).equal(expectedFileInputType, "Could not find file upload type");
    });

    it("should display court name input", () => {
      const courtName = formElements.getElementsByClassName(formGroupClass)[0];
      expect(courtName.innerHTML).contains(
        expectedCourtNameQuestion,
        "Could not find court name"
      );
      expect(courtName.innerHTML).contains(
        expectedCourtNameContainer,
        "Could not find court name container"
      );
    });

    it("should display sub list type question", () => {
      const listType = formElements
        .getElementsByClassName(insetTextClass)[0]
        .querySelector("#list-question");
      expect(listType.innerHTML).contains(
        expectedListType,
        "Could not find inset list type"
      );
    });
    ...
    it("should display content date question", () => {
      ...
      expect(contentDate.innerHTML).contains(
        expectedHearingDates,
    ...
    it("should display classification question", () => {
      const classification =
        formElements.getElementsByClassName(formGroupClass)[8];
      expect(classification.innerHTML).contains(
        expectedClassification,
        "Could not find classification question"
      );
    });

    it("should display language question", () => {
      const language = formElements.getElementsByClassName(formGroupClass)[9];
      expect(language.innerHTML).contains(
        expectedLanguage,
        "Could not find language question"
      );
    });

    it("should display display date question", () => {
      const displayDateFrom =
        formElements.getElementsByClassName(fieldSetClass)[1];
      const displayDateTo =
        formElements.getElementsByClassName(fieldSetClass)[2];
    ...
    it("should display page help heading", () => {
      ...
      expect(heading.innerHTML).contains(
        "Page Help",
        "Could not find page help heading"
      );
    });
    ...
    it("should display the available to heading", () => {
      ...
      expect(heading.innerHTML).contains(
        "Available to",
        "Could not find available to heading"
      );
    });
    ...
    it("should display the private heading", () => {
      ...
      expect(heading.innerHTML).contains(
        "Private",
        "Could not find private heading"
      );
    });
    ...
    it("should display the classified - heading", () => {
      ...
      expect(heading.innerHTML).contains(
        "Classified",
        "Could not find classified heading"
      );
    });
    ...
    it("should display the display from heading", () => {
      ...
      expect(heading.innerHTML).contains(
        "Display from",
        "Could not find display from heading"
      );
    });
    ...
    it("should display the display to heading", () => {
      ...
      expect(heading.innerHTML).contains(
        "Display to",
        "Could not find display to heading"
      );
    });
    ...
    it("should display the warning banner", () => {
      ...
      expect(warningHeader.innerHTML).contains(
        "Warning",
        "Could not find warning header"
      );
  ...
  describe("on POST", () => {
    ...
    it("should display court error", () => {
      const errorMessage = htmlRes.getElementsByClassName(
        "govuk-error-message"
      );
