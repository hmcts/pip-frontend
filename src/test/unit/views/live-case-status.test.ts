...
const rawData = fs.readFileSync(
  path.resolve(__dirname, "../mocks/liveCaseStatusUpdates.json"),
  "utf-8"
);
...
describe.skip("Live Status page", () => {
  ...
  it("should display correct court name", () => {
    ...
    expect(courtName[0].innerHTML).contains(
      expectedCourtName,
      "could not find the court name header"
    );
  });
  ...
  it("should contain expected column headings", () => {
    ...
    expect(tableHeaders[0].innerHTML).contains(
      "Court number",
      "Could not find court number header"
    );
    expect(tableHeaders[1].innerHTML).contains(
      "Case number",
      "Could not find case number header"
    );
    expect(tableHeaders[2].innerHTML).contains(
      "Case name",
      "Could not find case name header"
    );
    expect(tableHeaders[3].innerHTML).contains(
      "Status",
      "Could not find status header"
    );
  });
  ...
  it("should contain 5 rows, including the header", () => {
    ...
    expect(tableRows.length).equal(
      5,
      "Number of rows is not equal to expected amount"
    );
  });
  ...
  it("should contain a row with the correct values", () => {
    ...
    expect(items.item(0).innerHTML).contains(
      "1",
      "Court Number not found / correct"
    );
    expect(items.item(1).innerHTML).contains(
      "T20217099",
      "Case number not found / correct"
    );
    expect(items.item(2).innerHTML).contains(
      "Mills LLC's Hearing",
      "Case name not found / correct"
    );
    expect(items.item(3).innerHTML).contains(
      statusColumnValue,
  ...
  it("should display the link to go back to live case alphabet list", () => {
    ...
    expect(link.item(6).innerHTML).contains(
      "See another court or tribunal",
      "Link text is not present"
    );
    expect(link.item(6).getAttribute("href")).equal(
      "/live-case-alphabet-search",
      "Link value is not correct"
    );
  });
});
