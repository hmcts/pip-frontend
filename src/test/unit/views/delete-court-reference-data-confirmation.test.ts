...
describe("Delete Court Confirmation Page", () => {
  describe("without error", () => {
    ...
    it("should display warning message", () => {
      const warning = htmlRes.getElementsByClassName(
    ...
    it("should display summary table for court information", () => {
      const summaryKeys = htmlRes.getElementsByClassName(
        "govuk-summary-list__key"
      );
      const summaryValues = htmlRes.getElementsByClassName(
        "govuk-summary-list__value"
      );

      expect(summaryKeys[0].innerHTML).contains(
        "Court or Tribunal name",
        "Could not find court name header"
      );
      expect(summaryKeys[1].innerHTML).contains(
        "Location Type",
        "Could not find location type header"
      );
      expect(summaryKeys[2].innerHTML).contains(
        "Jurisdiction",
        "Could not find jurisdiction header"
      );
      expect(summaryKeys[3].innerHTML).contains(
        "Region",
        "Could not find region header"
      );

      expect(summaryValues[0].innerHTML).contains(
        "test court",
        "Could not find court name"
      );
      expect(summaryValues[1].innerHTML).contains(
        "location",
        "Could not find court location"
      );
      expect(summaryValues[2].innerHTML).contains(
        "testJ",
        "Could not find jurisdiction"
      );
      expect(summaryValues[3].innerHTML).contains(
        "testR",
    ...
    it("should display 2 radio options with valid values", () => {
      ...
      expect(radios[0].getAttribute("value")).equals(
        "yes",
        "Could not find valid radio value"
      );
      expect(radios[1].getAttribute("value")).equals(
        "no",
        "Could not find valid radio value"
      );
    });
