...
sinon
  .stub(ManualUploadService.prototype, "formatListRemovalValues")
  .returns(mockPublications);
...
describe("Remove List Summary Page", () => {
  ...
  it("should display results count", () => {
    ...
    expect(resultsCount.innerHTML).contains(
      "Showing 3 result(s)",
      "Could not find results paragraph"
    );
  });
  ...
  it("should display correct row values", () => {
    ...
    for (let i = 0; i < tableRows.length; i++) {
      ...
      expect(rowCells[1].innerHTML).contains(
        mockCourt.name,
        "Could not find valid court name"
      );
      expect(rowCells[2].innerHTML).contains(
        mockPublications[i].contDate,
        "Could not find valid contentDate"
      );
      expect(rowCells[3].innerHTML).contains(
        mockPublications[i].dateRange,
        "Could not find valid list date range"
      );
      expect(rowCells[4].innerHTML).contains(
        languageRowValues[i],
        "Could not find valid language"
      );
      expect(rowCells[5].innerHTML).contains(
        sensitivityValues[i],
        "Could not find valid sensitivity"
      );
      expect(rowCells[6].innerHTML).contains(
        "Remove",
        "Could not find valid action"
      );
