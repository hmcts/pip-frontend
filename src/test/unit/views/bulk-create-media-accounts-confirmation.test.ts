...
describe("Bulk Create Media Accounts Confirmation Page", () => {
  describe("without error", () => {
    ...
    it("should display correct table headers", () => {
      ...
      expect(headerNames[0].innerHTML).contains(
        "Email",
        "Table header does not match"
      );
      expect(headerNames[1].innerHTML).contains(
        "First Name",
        "Table header does not match"
      );
      expect(headerNames[2].innerHTML).contains(
        "Surname",
        "Table header does not match"
      );
    });
    ...
    it("should display correct row values", () => {
      ...
      for (let i = 0; i < tableRows.length; i++) {
        const rowCells =
          tableRows[i].getElementsByClassName("govuk-table__cell");
