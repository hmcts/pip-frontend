...
describe("File Upload Summary Page", () => {
  ...
  it("should display confirm button", async () => {
    ...
    expect(button.innerHTML).to.contain(
      "Confirm",
  ...
  it("should display correct summary keys and actions", async () => {
    ...
    for (let i = 0; i < summaryKeys.length; i++) {
      expect(keys[i].innerHTML).to.contain(
        summaryKeys[i],
      ...
      expect(
        actions[i].getElementsByClassName("govuk-link")[0].getAttribute("href")
      ).to.equal("manual-upload" + manualUploadLinks[i]);
    }
  });

  it("should display correct summary values", async () => {
    const formatContentDate = DateTime.fromFormat(
      mockData["content-date-from"],
      "dd/MM/yyyy HH:mm:ss"
    ).toFormat("d MMMM YYYY");
    const formatDisplayFromDate = DateTime.fromFormat(
      mockData["display-from"],
      "dd/MM/yyyy HH:mm:ss"
    ).toFormat("d MMMM YYYY");
    const formatDisplayToDate = DateTime.fromFormat(
      mockData["display-to"],
      "dd/MM/yyyy HH:mm:ss"
    ).toFormat("d MMMM YYYY");
    const values = htmlRes.getElementsByClassName("govuk-summary-list__value");
    expect(values[0].innerHTML).to.contain(
      mockData.court.courtName,
      "Court value not found"
    );
    ...
    expect(values[2].innerHTML).to.contain(
      mockData.listTypeName,
      "List type value not found"
    );
    expect(values[3].innerHTML).to.contain(
      formatContentDate,
      "Hearing start date value not found"
    );
    expect(values[4].innerHTML).to.contain(
      mockData.classificationName,
      "Classification values not found"
    );
    expect(values[5].innerHTML).to.contain(
      mockData.languageName,
