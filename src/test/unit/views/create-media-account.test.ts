  ...
  describe("without errors render", () => {
    ...
    it("should display full name input", () => {
      ...
      expect(fullNameLabel.innerHTML).contains(
        "Full name",
        "Could not find full name label"
      );
      expect(input.getAttribute("name")).equals(
        "fullName",
        "Could not find fullName input"
      );
    });
    ...
    it("should display employer input", () => {
      ...
      expect(employerLabel.innerHTML).contains(
        "Employer",
        "Could not find employer label"
      );
      expect(input.getAttribute("name")).equals(
        "employer",
        "Could not find employer input"
      );
    });
    ...
    it("should contain image upload", () => {
      ...
      expect(imageUpload.getAttribute("type")).equals(
        "file",
        "Could not find image upload"
      );
    ...
    it("should display t&c paragraph 1", () => {
      ...
      expect(message.innerHTML).contains(
        "A court and tribunal hearing account is granted based on you having " +
          "legitimate",
  ...
  describe("with errors", () => {
    ...
    it("should display error messages in th the dialog", () => {
      ...
      expect(listItems[3].innerHTML).contains(
        "There is a problem - We will need ID evidence to support " +
          "your application for an account",
        "Could not find file error"
      );
      expect(listItems[4].innerHTML).contains(
        "There is a problem - You must check the box to confirm you " +
