...
describe('Bulk Create Media Accounts Page', () => {
  describe('without error', () => {
    ...
    it('should have correct page title', () => {
      ...
      expect(pageTitle).contains('Bulk create media accounts',
        'Page title does not match');
    });

    it('should have correct header', () => {
      const heading = htmlRes.getElementsByClassName('govuk-heading-l');
      expect(heading[0].innerHTML).contains('Bulk create media accounts',
        'Header does not match');
    });

    it('should display correct messages in body', () => {
      const body = htmlRes.getElementsByClassName('govuk-body');
      expect(body[0].innerHTML).contains('Upload a csv file containing a list of media accounts to be created.',
  ...
  describe('with error', () => {
    ...
    it('should display error title in the summary', () => {
      ...
      expect(error[0].getElementsByClassName('govuk-error-summary__title')[0].innerHTML)
        .contains('There is a problem', 'Error title does not match');
    });
    ...
    it('should display error messages in the summary', () => {
      ...
      expect(errorItem.innerHTML).contains('File too large, please upload a file smaller than 2MB',
