...
const stubGetSubscriptions = sinon.stub(
...
describe("Subscription Configure List", () => {
  ...
  it("should display a back button with the correct value", () => {
    ...
    expect(backLink[0].innerHTML).contains(
      "Back",
      "Back button does not contain correct text"
    );
  ...
  it("should contain filter component", () => {
    ...
    expect(filterTitle[0].innerHTML).contains(
      "Filter",
  ...
  it("should contain selected filter component", () => {
    const filterTitle = htmlRes.getElementsByClassName("govuk-heading-m");
    expect(filterTitle[1].innerHTML).contains(
      "Selected filter",
      "Selected filter title does not exist"
    );
  });

  it("should contain apply filters button", () => {
    const applyFiltersButton = htmlRes.getElementsByClassName("govuk-button");
    expect(applyFiltersButton[0].innerHTML).contains(
      "Apply filters",
      "Apply filters button does not exist"
    );
  });

  it("should contain clear filters button", () => {
    const applyFiltersButton = htmlRes.querySelector(
      ".moj-filter__heading-action .govuk-link"
    );
    expect(applyFiltersButton.innerHTML).contains(
      "Clear filters",
      "Clear filters button does not exist"
    );
  });
  ...
  it("should contain 2 jurisdiction checkboxes", () => {
    ...
    expect(checkboxes.length).equal(
      2,
      "Could not find jurisdiction checkboxes"
    );
  });
  ...
  it("should contain a back to top link, that links back up to the top", () => {
    ...
    expect(backToTopButton.innerHTML).contains(
      "Back to top",
      "Back to top button does not exist"
    );
  ...
  it("should contain continue button", () => {
    ...
    expect(continueButton[3].innerHTML).contains(
      "Continue",
      "Continue button does not exist"
    );
  });

  it("should contain your selections component", () => {
    const selectionsTitle = htmlRes.getElementsByClassName("govuk-heading-m");
    expect(selectionsTitle[2].innerHTML).contains(
      "Your selection(s)",
      "Your selections title does not exist"
    );
  });
  ...
  it("should contain selections counter", () => {
    ...
    expect(selectionText[2].innerHTML).contains(
      "selected",
      "Selection text does not exist"
    );
  ...
  it("should contain letters that navigate to other sections of the page", () => {
    const alphabeticalLetters =
      htmlRes.getElementsByClassName("two-rows-alphabet");
    const letterA = htmlRes.getElementById("A-selector");
    expect(letterA.innerHTML).contains("A", "Alphabetical link is not present");
    expect(alphabeticalLetters.length).equal(
      26,
      "Could not find alphabet letters"
    );
  });
