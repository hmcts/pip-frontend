...
describe('Crime rejected login page', () => {
    beforeAll(async () => {
        const PAGE_URL = '/crime-rejected-login';
        await request(app)
            .get(PAGE_URL)
            .then(res => {
    ...
    it('should display paragraph 1 text', () => {
        ...
        expect(bodyText[4].innerHTML).contains(
            'You have successfully signed into your Common Component account. Unfortunately, your account role does not allow you to access the verified user part of the Court and tribunal hearings service',
    ...
    it('should display paragraph 2 text', () => {
        ...
        expect(bodyText[5].innerHTML).contains(
            'Solicitors and other legal professionals are the only users allowed to sign in with a Common Component account. For more information about MyHMCTS the link below:',
            'Could not find body text'
        );
    });
    ...
    it('should display the link', () => {
        ...
        expect(link[3].textContent).contains(
            'Common Component: online case management for legal professionals',
            'Could not find link'
        );
