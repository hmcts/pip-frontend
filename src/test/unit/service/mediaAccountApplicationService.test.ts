...
describe("Summary Of Publications Service", () => {
  ...
  const mediaApplicationByIdStub = sinon.stub(
    AccountManagementRequests.prototype,
    "getMediaApplicationById"
  );
  const mediaApplicationByImageStub = sinon.stub(
    AccountManagementRequests.prototype,
    "getMediaApplicationImageById"
  );
  const mediaApplicationUpdateStatusStub = sinon.stub(
    AccountManagementRequests.prototype,
    "updateMediaApplicationStatus"
  );
  const createAccountServiceStub = sinon.stub(
  ...
  it("should return the expected application by id", async () => {
    ...
    const application = await accountApplicationService.getApplicationById(
      applicationId
    );
  ...
  it("should return the expected application by id and status", async () => {
    ...
    const application =
      await accountApplicationService.getApplicationByIdAndStatus(
        applicationId,
        "PENDING"
      );
    expect(application).toStrictEqual(formattedApplication);
  ...
  it("should return the expected image", async () => {
    ...
    const applicationImage = await accountApplicationService.getImageById(
      imageId
    );
  ...
  it("should return the expected application when successfully created", async () => {
    ...
    createAccountServiceStub
      .withArgs(applicationId, adminEmail)
      .resolves(dummyApplication);
    mediaApplicationUpdateStatusStub
      .withArgs(applicationId, "APPROVED")
      .resolves(approvedApplication);

    const application =
      await accountApplicationService.createAccountFromApplication(
        applicationId,
        adminEmail
      );
