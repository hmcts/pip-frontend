...
describe("Third Party Service tests", () => {
  ...
  describe("generateListTypes", () => {
    ...
    const generatedListTypes = thirdPartyService.generateListTypes(
      listTypes,
      subscriptions
    );

    expect(Object.keys(generatedListTypes).length).equals(
      2,
      "Number of list types does not match expected types"
    );

    expect(generatedListTypes["LIST_A"].listFriendlyName).equals(
      "List A",
      "List Friendly Name is not as expected"
    );

    expect(generatedListTypes["LIST_A"].checked).equals(
      false,
      "Checked property not as expected"
    );

    expect(generatedListTypes["LIST_B"].checked).equals(
      true,
      "Checked property not as expected"
    );
  });
  ...
  describe("getThirdPartyAccounts", () => {
    ...
    const getThirdPartyAccountsStub = sinon.stub(
    ...
    it("should return correct number and details of third party objects", async () => {
      ...
      expect(data.length).to.equal(
        2,
        "Number of accounts returned does not match expected length"
      );

      const firstAccount = data[0];

      expect(firstAccount["userId"]).to.equal(
        "1234-1234",
        "User ID does not match expected ID"
      );
      expect(firstAccount["provenanceUserId"]).to.equal(
        "ThisIsAName",
        "Provenance User ID does not match expected ID"
      );
      expect(firstAccount["roles"]).to.equal(
        "GENERAL_THIRD_PARTY",
  ...
  describe("generateAvailableChannels", () => {
    it("should return correct checked options when existing subscription", () => {
      ...
      const channels = thirdPartyService.generateAvailableChannels(
        subscriptionChannels,
        subscriptions
      );

      expect(channels.length).to.equal(
        2,
        "Unexpected number of channels returned"
      );
      expect(channels[0].value).to.equal(
        "CHANNEL_A",
        "Unexpected channel value returned"
      );
      expect(channels[0].text).to.equal(
        "CHANNEL_A",
        "Unexpected channel text returned"
      );
      expect(channels[0].checked).to.equal(
        true,
        "Unexpected checked value returned"
      );
      expect(channels[1].checked).to.equal(
        false,
        "Unexpected checked value returned"
      );
    });
  ...
  describe("create third party subscription", () => {
    ...
    it("check a subscription is created", () => {
      ...
      thirdPartyService.createdThirdPartySubscription(
        adminUserId,
        userId,
        listType,
        channel
  ...
  describe("get third party by user ID", () => {
    ...
    it("check user is returned", async () => {
      ...
      const returnedUser = await thirdPartyService.getThirdPartyUserById(
        userId,
        "1234-1234"
      );

      expect(returnedUser["userId"]).to.equal(
        userId,
        "User ID not as expected"
      );
      expect(returnedUser["createdDate"]).to.equal(
        "18 November 2022",
        "Formatted date not as expected"
      );
      expect(returnedUser["userProvenance"]).to.equal(
        "THIRD_PARTY",
        "User provenance not as expected"
      );
    });
  ...
  describe("handle third party subscription update", () => {
    ...
    it("check that subscribe is called for a brand new subscription", async () => {
      ...
      getSubscriptionsStub
        .withArgs(userId)
        .resolves({ listTypeSubscriptions: [] });
    ...
    it("check that unsubscribe is called if no longer selected", async () => {
      ...
      getSubscriptionsStub.withArgs(userId).resolves({
        listTypeSubscriptions: [
          { listType: "LIST_A", channel: "CHANNEL_B", subscriptionId: "2345" },
        ],
      });
