...
const publicationRequestStub = sinon.stub(
  PublicationRequests.prototype,
  "getPublicationByCaseValue"
);
...
sinon
  .stub(publicationRequests, "getPubsPerLocation")
...
describe("Publication service", () => {
  it("should return array of Search Objects based on partial case name", async () => {
    const results = await publicationService.getCasesByCaseName(
      caseNameValue,
      userId
    );
  ...
  it("should return Search Object matching case number", async () => {
    expect(
      await publicationService.getCaseByCaseNumber(caseNumberValue, userId)
    ).to.equal(returnedArtefact[0].search.cases[0]);
  });

  it("should return Search Object matching case urn", async () => {
    expect(
      await publicationService.getCaseByCaseUrn(caseUrnValue, userId)
    ).to.equal(returnedArtefact[0].search.cases[1]);
  });

  it("should return null processing failed request", async () => {
    expect(
      await publicationService.getCaseByCaseUrn("invalid", userId)
    ).is.equal(null);
  });
  ...
  it("should return list types", () => {
    ...
    expect(sjpResult["jurisdictions"]).to.deep.equal([
  ...
  describe("getIndivPubJson Service", () => {
    it("should return publication json", () => {
      return publicationService
        .getIndividualPublicationJson("", userId)
        .then((data) => {
  ...
  describe("getIndivPubMetadata Publication Service", () => {
    it("should return publication meta object", () => {
      return publicationService
        .getIndividualPublicationMetadata("", userId)
        .then((data) => {
  ...
  describe("Language to load the page in", () => {
    it("should return english if the user is english and the list is english", () => {
      expect(publicationService.languageToLoadPageIn("ENGLISH", "en")).to.equal(
        "en"
      );
    });

    it("should return bilingual if the user is english and the list is welsh", () => {
      expect(publicationService.languageToLoadPageIn("WELSH", "en")).to.equal(
        "bill"
      );
    });
    ...
    it("should return bilingual if the user is english and the list is bilingual", () => {
      expect(
        publicationService.languageToLoadPageIn("BI_LINGUAL", "en")
      ).to.equal("bill");
    });

    it("should return welsh if the user is welsh and the list is welsh", () => {
      expect(publicationService.languageToLoadPageIn("WELSH", "cy")).to.equal(
        "cy"
      );
    });

    it("should return bilingual if the user is welsh and the list is english", () => {
      expect(publicationService.languageToLoadPageIn("ENGLISH", "cy")).to.equal(
        "bill"
      );
    });
    ...
    it("should return bilingual if the user is welsh and the list is bilingual", () => {
      expect(
        publicationService.languageToLoadPageIn("BI_LINGUAL", "cy")
      ).to.equal("bill");
    });
  });
});
