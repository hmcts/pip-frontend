...
sinon
  .stub(AccountManagementRequests.prototype, "getAllAccountsExceptThirdParty")
  .resolves(testApiResponseData);
...
describe("User management service", () => {
  ...
  it("should build user update select box", () => {
    const response =
      userManagementService.buildUserUpdateSelectBox("SYSTEM_ADMIN");
  ...
  it("should generate the filter key values", () => {
    ...
    const response = userManagementService.generateFilterKeyValues(
      JSON.parse(JSON.stringify(testReqBody))
    );
  ...
  it("should build the manage user summary list for an admin user", () => {
    const response = userManagementService.buildManageUserSummaryList(
      testManageUserSummaryAdmin
    );

    expect(response["rows"][0]["key"]["text"]).to.contain("User ID");
    expect(response["rows"][0]["value"]["text"]).to.contain(
      testManageUserSummaryAdmin.userId
    );
    expect(response["rows"][1]["key"]["text"]).to.contain("Email");
    expect(response["rows"][1]["value"]["text"]).to.contain(
      testManageUserSummaryAdmin.email
    );
    ...
    expect(response["rows"][4]["value"]["text"]).to.contain(
      testManageUserSummaryAdmin.provenanceUserId
    );
    expect(response["rows"][5]["key"]["text"]).to.contain("Creation Date");
    expect(response["rows"][5]["value"]["text"]).to.contain(
      "05/11/2022 18:45:37"
    );
    expect(response["rows"][6]["key"]["text"]).to.contain("Last Sign In");
    expect(response["rows"][6]["value"]["text"]).to.contain(
      "07/11/2022 18:45:37"
    );
  });
  ...
  it("should build the manage user summary list for a media user", () => {
    const response = userManagementService.buildManageUserSummaryList(
      testManageUserSummaryMedia
    );

    expect(response["rows"][0]["key"]["text"]).to.contain("User ID");
    expect(response["rows"][0]["value"]["text"]).to.contain(
      testManageUserSummaryMedia.userId
    );
    expect(response["rows"][1]["key"]["text"]).to.contain("Email");
    expect(response["rows"][1]["value"]["text"]).to.contain(
      testManageUserSummaryMedia.email
    );
    expect(response["rows"][2]["key"]["text"]).to.contain("Role");
    ...
    expect(response["rows"][4]["key"]["text"]).to.contain("Provenance ID");
    expect(response["rows"][4]["value"]["text"]).to.contain(
      testManageUserSummaryMedia.provenanceUserId
    );
    expect(response["rows"][5]["key"]["text"]).to.contain("Creation Date");
    expect(response["rows"][5]["value"]["text"]).to.contain(
      "05/11/2022 18:45:37"
  ...
  it("should return formatted data from the getFormattedData function", async () => {
    ...
    // Pagination data
    expect(response["paginationData"]["previous"].labelText).to.equal(
      "1 of 10"
    );
