...
const pubStub = sinon.stub(
  PublicationRequests.prototype,
  "getPublicationsByCourt"
);
const fileStub = sinon.stub(
  PublicationRequests.prototype,
  "getIndividualPublicationFile"
);
const metaStub = sinon.stub(
  PublicationRequests.prototype,
...
describe("Summary Of Publications Service", () => {
  it("should return a list of publications", async () => {
    ...
    expect(await sopService.getPublications(0, userId)).to.equal(
      '{"item":"listOfPubs"}'
    );
  });
  it("should return metadata", async () => {
    ...
    expect(
      await pubService.getIndividualPublicationMetadata(0, userId)
    ).to.equal('{"item":"listOfMetadata"}');
  });
  it("should return file", async () => {
    fileStub.withArgs(0).resolves('{"item":"file"}');
    expect(await pubService.getIndividualPublicationFile(0, userId)).to.equal(
      '{"item":"file"}'
    );
  });
  it("should return json", async () => {
    jsonStub.withArgs(0).resolves('{"item":"json"}');
    expect(await pubService.getIndividualPublicationJson(0, userId)).to.equal(
      '{"item":"json"}'
    );
  });
});
