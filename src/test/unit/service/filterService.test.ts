...
sinon
  .stub(LocationService.prototype, "generateAlphabetisedAllCourtList")
  .resolves(listData);
sinon
  .stub(LocationService.prototype, "generateFilteredAlphabetisedCourtList")
  .resolves([listData[0]]);
...
describe("Filter Service", () => {
  it("should build filter header options for checkboxes", () => {
    expect(
      Object.keys(filterService.buildFilterValueOptions(listData, [])).length
    ).toBe(2);
  });
  ...
  it("should remove item in array", () => {
    expect(
      filterService.handleFilterClear(["test", "removed"], "removed")
    ).toStrictEqual(["test"]);
  });

  it("should remove item in array leaving empty", () => {
    expect(
      filterService.handleFilterClear(["removed"], "removed")
    ).toStrictEqual([]);
  });

  it("should return both Jurisdiction and Region", () => {
    expect(
      filterService.splitFilters(filterNames, requestFilters)
    ).toStrictEqual(requestFilters);
  });

  it("should return only Region", () => {
    expect(
      filterService.splitFilters(filterNames, requestFiltersNoJurisdiction)
    ).toStrictEqual({ Jurisdiction: "", Region: "Bedford" });
  });

  it("should return only Jurisdiction", () => {
    expect(
      filterService.splitFilters(filterNames, requestFiltersNoRegion)
    ).toStrictEqual({ Jurisdiction: "Crown,Tribunal", Region: "" });
  });

  it("should find and return both Jurisdiction and Region", () => {
    expect(
      filterService.findAndSplitFilters(filterValues, allFilterOptions)
    ).toStrictEqual({ Jurisdiction: "Tribunal", Region: "Wales" });
  });
  ...
  it("should return object for rendering with no clear or filters selected", async () => {
    expect(
      await filterService.handleFilterInitialisation(
        null,
        null,
        englishLanguage
      )
  ...
  it("should return object for rendering with no clear or filters selected for welsh", async () => {
    expect(
      await filterService.handleFilterInitialisation(null, null, welshLanguage)
    ).toStrictEqual({
      alphabetisedList: listData,
      filterOptions: { ...filterService.buildFilterValueOptions(listData, []) },
    });
  ...
  it("should return all courts when clear all has been passed for welsh", async () => {
    expect(
      await filterService.handleFilterInitialisation("all", null, welshLanguage)
    ).toStrictEqual({
      alphabetisedList: listData,
      filterOptions: { ...filterService.buildFilterValueOptions(listData, []) },
    });
  ...
  it("should return filter values from a key", () => {
    ...
    expect(filterService.generateFilterKeyValues(body)).toStrictEqual([
      "test",
      "val",
    ]);
  });
