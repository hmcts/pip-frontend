...
const stubUserSubscription = sinon.stub(
...
stubUserSubscription
  .withArgs(userIdWithSubscriptions)
  .returns(subscriptionResult2.data);
...
const publicationStub = sinon.stub(
...
const bulkDeleteStub = sinon.stub(
  SubscriptionRequests.prototype,
...
subscriptionStub
  .withArgs(caseSubscriptionPayload, "courts", "1")
  .resolves(true);
...
cacheSetStub
  .withArgs([mockCourt], "courts", userIdWithSubscriptions)
  .resolves();
...
bulkDeleteStub
  .withArgs(["ValidSubscriptionId"])
  .resolves("Subscription was deleted");
bulkDeleteStub.withArgs(["InValidSubscriptionId"]).resolves(null);
updateListTypeSubscriptionStub
  .withArgs("1", courtSubscriptionWithSingleListTypePayload)
  .resolves(true);
updateListTypeSubscriptionStub
  .withArgs("1", courtSubscriptionWithMultipleListTypePayload)
  .resolves(true);
updateListTypeSubscriptionStub
  .withArgs("1", courtSubscriptionWithEmptyListTypePayload)
  .resolves(true);
updateListTypeSubscriptionStub
  .withArgs(null, courtSubscriptionWithEmptyListTypePayload)
  .resolves(false);
...
describe("getSubscriptionDataForView function", () => {
  ...
  describe("for Subscription Management page", () => {
    ...
    it("should sort location subscription data by court name", async () => {
      ...
      expect(subscriptionData.locationTableData[0][0].text).toEqual(
        "Aberdeen Tribunal Hearing Centre"
      );
      expect(subscriptionData.locationTableData[1][0].text).toEqual(
        "Barkingside Magistrates' Court"
      );
      expect(subscriptionData.locationTableData[2][0].text).toEqual(
...
describe("getCaseDetails function", () => {
  it("should return case details list", async () => {
    const caseDetailsList = await subscriptionService.getCaseDetails(
      ["T485914"],
      user
    );
...
describe("subscribe function", () => {
  it("should return true for successful subscription when no subscriptions", async () => {
    const subscriptionRes = await subscriptionService.subscribe(
      userIdWithoutSubscriptions
    );
...
describe("createSubscriptionPayload function", () => {
  it("should create court subscription payload", async () => {
    const payload = subscriptionService.createSubscriptionPayload(
      mockCourt,
      "courts",
      "1"
    );
...
describe("configureListTypeForLocationSubscriptions", () => {
  it("should return a message if list type subscription is updated", async () => {
    const result =
      await subscriptionService.configureListTypeForLocationSubscriptions(
        "1",
        "CIVIL_DAILY_CAUSE_LIST"
      );
  ...
  it("should return a message if multiple list type subscription is updated", async () => {
    const listTypeArray =
      "CIVIL_DAILY_CAUSE_LIST,FAMILY_DAILY_CAUSE_LIST".split(",");
    const result =
      await subscriptionService.configureListTypeForLocationSubscriptions(
        "1",
        listTypeArray
...
describe("unsubscribing", () => {
  it("should return a message if subscription is deleted", async () => {
    const payload = await subscriptionService.unsubscribe(
      "ValidSubscriptionId",
      "2345-2345"
    );
...
describe("bulkDeleteSubscriptions", () => {
  it("should return a message if subscription is deleted", async () => {
    const payload = await subscriptionService.bulkDeleteSubscriptions([
      "ValidSubscriptionId",
    ]);
    expect(payload).toEqual("Subscription was deleted");
  });
...
describe("generateListTypesForCourts", () => {
  ...
  const subscriptionData = fs.readFileSync(
    path.resolve(
      __dirname,
      "../../../test/unit/mocks/listTypeSubscriptions/listTypeSubscriptions.json"
    ),
    "utf-8"
  );
  ...
  it("generate list types with no filters with no selected", async () => {
    ...
    const civilAndFamilyCauseList =
      listTypes["CIVIL_AND_FAMILY_DAILY_CAUSE_LIST"];
  ...
  it("retrieve subscription channels", async () => {
    const subscriptionChannelStub = sinon.stub(
      SubscriptionRequests.prototype,
      "retrieveSubscriptionChannels"
    );
