...
const rawCrimePartyData = fs.readFileSync(
  path.resolve(__dirname, "../../mocks/crimeListParty.json"),
  "utf-8"
);
...
describe("Crime Data manipulation service", () => {
  describe("CrimeListsDataManipulatedService", () => {
    ...
    it("should formatted the party information correctly for prosecution authority and defendant", async () => {
      ...
      expect(
        data["courtLists"][0]["courtHouse"]["courtRoom"][0]["session"][0][
          "sittings"
        ][0]["hearing"][0]["defendant"]
      ).to.equal("Defendant_SN, Defendant_FN");
      expect(
        data["courtLists"][0]["courtHouse"]["courtRoom"][0]["session"][0][
          "sittings"
        ][0]["hearing"][0]["prosecutingAuthority"]
      ).to.equal("Pro_Auth");
    });
    ...
    it("should be able to find linked cases for a particular case", async () => {
      ...
      expect(
        data["courtLists"][0]["courtHouse"]["courtRoom"][0]["session"][0][
          "sittings"
        ][0]["hearing"][0]["case"][0]["linkedCases"]
      ).to.equal("caseid111, caseid222");
      expect(
        data["courtLists"][0]["courtHouse"]["courtRoom"][0]["session"][0][
          "sittings"
        ][0]["hearing"][0]["case"][1]["linkedCases"]
      ).to.equal("");
    });
  ...
  describe("manipulateParty", () => {
    ...
    it("should format single defendant and prosecuting authority", async () => {
      ...
      expect(hearing.prosecutingAuthority).to.equal(
    ...
    it("should format multiple defendants and prosecuting authorities", async () => {
      ...
      crimeListsService.manipulateParty(hearing);
      expect(hearing.defendant).to.equal(
        "SurnameA, ForenamesA, SurnameB, ForenamesB"
      );
      expect(hearing.defendantRepresentative).to.equal(
        "Defendant rep nameA, Defendant rep nameB"
      );
      expect(hearing.prosecutingAuthority).to.equal(
        "Prosecuting authority nameA, Prosecuting authority nameB"
      );
    });
  });
});
