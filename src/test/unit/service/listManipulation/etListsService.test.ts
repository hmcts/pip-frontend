...
const etDailyListData = fs.readFileSync(
  path.resolve(__dirname, "../../mocks/etDailyList-withdates.json"),
  "utf-8"
);
...
describe("Reshaped ET Fortnightly List - splitting data from a courtroom format to a day by day view.", () => {
  it("should return ET Fortnightly List", async () => {
    const data = await etListsService.reshapeEtFortnightlyListData(
      etDailyListData,
      "en"
    );
  ...
  it("should match the completed mock", async () => {
    ...
    expect(JSON.stringify(data)).to.contain(
      "Lord H. Bouffant, LEGALADVISOR: Dame H. Wiggins"
    );
  });

  it("should have data for two courthouses", async () => {
    const data = JSON.stringify(
      await etListsService.reshapeEtFortnightlyListData(etDailyListData, "en")
    );
  ...
  it("should have exactly one record for Leicester", async () => {
    ...
    expect(JSON.stringify(data).match("Leicester Crown Court").length).to.equal(
      1
    );
  });
  ...
  it("should have exactly four records for Nottingham", async () => {
    ...
    expect(
      data.match(new RegExp("Nottingham Justice Centre", "g")).length - 1
    ).to.equal(4);
  });
