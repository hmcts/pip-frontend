...
describe('Crown Advance PDDA List service', () => {
    describe('processPayload', () => {
        const payload = fs.readFileSync(path.resolve(__dirname, '../../mocks/crownAdvancePddaList.json'), 'utf-8');
        const parsedPayload = JSON.parse(payload);
        it('should return all hearing types', async () => {
            const data = crownAdvancePddaListService.processPayload(parsedPayload);
        ...
        it('should format dependant name', async () => {
            const data: Map<string, object[]> = await crownAdvancePddaListService.processPayload(parsedPayload);
