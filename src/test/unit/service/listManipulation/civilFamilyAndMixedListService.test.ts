...
const expectedApplicant =
  "Surname, LEGALADVISOR: Mr Individual Forenames Individual Middlename Individual Surname";
...
const rawFamilyDailyCauseWithReorderedPartyMappings = fs.readFileSync(
  path.resolve(
    __dirname,
    "../../mocks/familyDailyCauseListWithReorderedPartyMappings.json"
  ),
  "utf-8"
);
const rawDailyCauseData = fs.readFileSync(
...
describe("Tests for the civil, family and mixed lists service.", function () {
  describe("manipulatedDailyListData", () => {
    ...
    it("should return daily cause list object", async () => {
      const data = await service.sculptedCivilFamilyMixedListData(
        rawDailyCauseData
      );
    ...
    it("should calculate duration more than one hour of Hearing in cause list object", async () => {
      ...
      expect(
        data["courtLists"][1]["courtHouse"]["courtRoom"][0]["session"][0][
          "sittings"
        ][0]["durationAsHours"]
      ).to.equal(1);
      expect(
    ...
    it("should calculate duration is one hour of Hearing in cause list object", async () => {
      ...
      expect(
        data["courtLists"][2]["courtHouse"]["courtRoom"][0]["session"][0][
          "sittings"
        ][0]["durationAsHours"]
      ).to.equal(1);
      expect(
    ...
    it("should calculate duration less than a hour of Hearing in cause list object", async () => {
      ...
      expect(
        data["courtLists"][3]["courtHouse"]["courtRoom"][0]["session"][0][
          "sittings"
        ][0]["durationAsHours"]
      ).to.equal(0);
      expect(
    ...
    it("should set caseHearingChannel to sitting channel", async () => {
      const data = await service.sculptedCivilFamilyMixedListData(
        rawFamilyDailyCauseData
      );
      expect(
        data["courtLists"][0]["courtHouse"]["courtRoom"][0]["session"][0][
          "sittings"
        ][0]["caseHearingChannel"]
      ).to.equal("testSittingChannel");
    });

    it("should set sessionChannel to sitting channel", async () => {
      familyDailyCause["courtLists"][0]["courtHouse"]["courtRoom"][0][
        "session"
      ][0]["sittings"][0]["channel"] = [];
      const data = await service.sculptedCivilFamilyMixedListData(
        JSON.stringify(familyDailyCause)
      );
      expect(
        data["courtLists"][0]["courtHouse"]["courtRoom"][0]["session"][0][
          "sittings"
    ...
    it("should concat judiciaries with no presiding", async () => {
      familyDailyCause["courtLists"][0]["courtHouse"]["courtRoom"][0][
        "session"
      ][0]["judiciary"][1]["isPresiding"] = false;
      const data = await service.sculptedCivilFamilyMixedListData(
        JSON.stringify(familyDailyCause)
      );
      expect(
      ...
      ).to.equal(nonPresidingJudiciary);
    });
    ...
    it("should build the applicants and the respondents of the party", async () => {
      ...
      expect(
        data["courtLists"][0]["courtHouse"]["courtRoom"][0]["session"][0][
          "sittings"
        ][0]["hearing"][0]["applicant"]
      ).to.equal(expectedApplicant);
      expect(
        data["courtLists"][0]["courtHouse"]["courtRoom"][0]["session"][0][
          "sittings"
        ][0]["hearing"][0]["respondent"]
    ...
    it("should build when we have multiple applicants and the respondents of the party", async () => {
      ...
      expect(
        data["courtLists"][0]["courtHouse"]["courtRoom"][0]["session"][0][
          "sittings"
        ][1]["hearing"][0]["applicant"]
      ).to.equal(expectedMultipleApplicant);
      expect(
        data["courtLists"][0]["courtHouse"]["courtRoom"][0]["session"][0][
          "sittings"
        ][1]["hearing"][0]["respondent"]
    ...
    it("should build the applicants and the respondents of the party with data that requires mapping", async () => {
      const data = await service.sculptedCivilFamilyMixedListData(
        rawFamilyDailyCausePartyMappingData
      );
      expect(
        data["courtLists"][0]["courtHouse"]["courtRoom"][0]["session"][0][
          "sittings"
    ...
    it("should build only the applicants and the respondents representative of the party", async () => {
      ...
      expect(
        data["courtLists"][0]["courtHouse"]["courtRoom"][0]["session"][0][
          "sittings"
        ][0]["hearing"][1]["applicant"]
      ).to.equal("LEGALADVISOR: Individual Surname");
      expect(
        data["courtLists"][0]["courtHouse"]["courtRoom"][0]["session"][0][
          "sittings"
        ][0]["hearing"][1]["respondent"]
      ).to.equal("LEGALADVISOR: Individual Surname");
    });
    ...
    it("should build only the applicants and the respondents of the party", async () => {
      ...
      expect(
        data["courtLists"][0]["courtHouse"]["courtRoom"][0]["session"][0][
          "sittings"
        ][0]["hearing"][2]["applicant"]
      ).to.equal("Individual Surname");
      expect(
        data["courtLists"][0]["courtHouse"]["courtRoom"][0]["session"][0][
          "sittings"
        ][0]["hearing"][2]["respondent"]
      ).to.equal("Individual Surname");
    });
    ...
    it("when there is no party information provided", async () => {
      ...
      expect(
        data["courtLists"][0]["courtHouse"]["courtRoom"][0]["session"][0][
          "sittings"
        ][0]["hearing"][3]["applicant"]
      ).to.equal(undefined);
      expect(
        data["courtLists"][0]["courtHouse"]["courtRoom"][0]["session"][0][
          "sittings"
        ][0]["hearing"][3]["respondent"]
      ).to.equal(undefined);
    });
    ...
    it("when there is reordered party mappings in the array, it still provides the correct mappings", async () => {
      ...
      expect(
        ...
        "Surname, LEGALADVISOR: Mr Forenames Middlename SurnameApplicant"
      );
      expect(
        data["courtLists"][0]["courtHouse"]["courtRoom"][0]["session"][0][
          "sittings"
        ...
        "Surname, LEGALADVISOR: Mr Forenames Middlename SurnameRespondent"
      );
    });
  });
});
