...
describe("IAC Daily List service", () => {
  describe("manipulateIacDailyListData", () => {
    const rawIacDailyListData = fs.readFileSync(
      path.resolve(__dirname, "../../mocks/iacDailyList.json"),
      "utf-8"
    );

    it("should return all court lists", async () => {
      const data = await iacService.manipulateIacDailyListData(
        rawIacDailyListData,
        "en"
      );
    ...
    it("should format multiple judiciaries", async () => {
      ...
      ).to.equal("Judge Jacobs, Magistrate Jones, Magistrate Patel");
    });
    ...
    it("should format hearing parties with multiple appellants and respondents", async () => {
      ...
      expect(hearing["appellant"]).to.equal(
        "Mr Steve M Taylor, Mrs Rose Taylor"
      );
      expect(hearing["appellantRepresentative"]).to.equal("JCT Law Centre");
      expect(hearing["prosecutingAuthority"]).to.equal(
        "Entry Clearance Officer, Secretary of State"
      );
    });
    ...
    it("should format single linked case", async () => {
      ...
      expect(
        data["courtLists"][0]["courtHouse"]["courtRoom"][0]["session"][0][
          "sittings"
        ][0]["hearing"][0]["case"][0]["formattedLinkedCases"]
      ).to.equal("1234");
    });
    ...
    it("should format multiple linked cases", async () => {
      ...
      expect(
        data["courtLists"][0]["courtHouse"]["courtRoom"][0]["session"][0][
          "sittings"
        ][0]["hearing"][1]["case"][0]["formattedLinkedCases"]
