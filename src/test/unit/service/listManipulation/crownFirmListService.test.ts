...
const rawCrownFirmData = fs.readFileSync(
  path.resolve(__dirname, "../../mocks/crownFirmSemiDigested.json"),
  "utf-8"
);
...
describe("Crown firm list splitter service", () => {
  describe("Crown firm list splitter service", () => {
    ...
    it("should split the initial json file down to two courtHouses", async () => {
      ...
      expect(data[0].courtName).equal(
        "Colchester Social Security and Child Support"
      );
      expect(data[1].courtName).equal(
        "Glasgow Social Security and Child Support"
      );
    });
