...
sinon
  .stub(DataManagementRequests.prototype, "uploadPublication")
  .resolves(true);
sinon
  .stub(DataManagementRequests.prototype, "uploadJSONPublication")
  .resolves(true);
sinon
  .stub(DataManagementRequests.prototype, "uploadLocationFile")
  .resolves(true);
...
describe("Manual upload service", () => {
  ...
  describe("Form validation", () => {
    ...
    it("should return null if all validated fields pass", async () => {
      expect(
        await manualUploadService.validateFormFields(
          formValues,
          englishLanguage,
          languageFile
        )
    ...
    it("should return invalid court error message", async () => {
      ...
      const errors = await manualUploadService.validateFormFields(
        formValues,
        englishLanguage,
        languageFile
      );
      expect(errors["courtError"]).to.equal(
        "Please enter and select a valid court"
      );
    });
    ...
    it("should return error when invalid display date from is passed", async () => {
      ...
      expect(errors["displayDateError"]["from"]).to.equal(
    ...
    it("should return error when invalid display date to is passed", async () => {
      ...
      expect(errors["displayDateError"]["to"]).to.equal(
        "Please enter a valid date"
      );
    });
    ...
    it("should return error when invalid date range is passed", async () => {
      ...
      expect(errors["displayDateError"]["range"]).to.equal(
        "Please make sure 'to' date is after 'from' date"
      );
    });
    ...
    it("should formatted date-from date correctly", async () => {
      ...
      formValues["display-date-from-year"] = "2022";
      const data = await manualUploadService.buildDate(
        formValues,
        "display-date-from"
      );
  ...
  it("should generate headers object", () => {
    const builtHeaders =
      manualUploadService.generatePublicationUploadHeaders(headers);
    expect(builtHeaders).to.deep.equal(expectedHeaders);
  });

  it("should upload a publication", async () => {
    const fileUpload = await manualUploadService.uploadPublication(
      headers,
      true
    );
  ...
  it("should upload a reference data file", async () => {
    const fileUpload = await manualUploadService.uploadLocationDataPublication(
      headers
    );
    expect(fileUpload).to.be.true;
  });
  ...
  it("should formatted date-from and date-to correctly", async () => {
    const data = await manualUploadService.formatPublicationDates(
      headers,
      true
    );
  ...
  it("should return court id and name as object", async () => {
    expect(
      await manualUploadService.appendlocationId("validCourt", englishLanguage)
    ).to.deep.equal({ courtName: "validCourt", locationId: 1 });
  });
  ...
  describe("formatting list removal", () => {
    ...
    it("should return formatted list for a valid input list", () => {
      const list =
        manualUploadService.formatListRemovalValues(validRemoveListInput);
