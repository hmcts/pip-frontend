...
stubCourtDeletion
  .withArgs(3, requester)
  .returns({ isExists: false, errorMessage: "" });

describe("Court Service", () => {
  it("should return all courts", async () => {
    expect(await courtService.fetchAllLocations(englishLanguage)).to.equal(
      hearingsData
    );
  });
  ...
  it("should return found court for court name match", async () => {
    expect(
      await courtService.getLocationByName(validCourt, englishLanguage)
    ).to.equal(hearingsData[0]);
  });

  it("should return found court for welsh court name match", async () => {
    expect(
      await courtService.getLocationByName(validWelshCourt, welshLanguage)
    ).to.equal(hearingsData[0]);
  });
  ...
  it("should return null for no name match", async () => {
    ...
    expect(
      await courtService.getLocationByName("test", englishLanguage)
    ).to.equal(null);
  });
  ...
  it("should return null for no name match", async () => {
    ...
    expect(
      await courtService.getLocationByName("test", welshLanguage)
    ).to.equal(null);
  });

  it("should return found court name in for english", async () => {
    expect(
      await courtService.findCourtName(
        hearingsData[0],
        englishLanguage,
        englishLanguageFile
      )
    ).to.equal("Abergavenny Magistrates' Court");
  });
  ...
  it("should return missing court for english if court is not found", async () => {
    ...
    ).to.equal("Missing Court");
  });

  it("should return missing court for welsh if court is not found", async () => {
    expect(
      await courtService.findCourtName(null, welshLanguage, englishLanguageFile)
    ).to.equal("Llys ar Goll");
  });
  ...
  it(`should have filtered a ${validCourt} key`, async () => {
    const data = await courtService.generateFilteredAlphabetisedCourtList(
      "",
      "Crown",
      englishLanguage
    );
  ...
  it("should return sorted courts list", () => {
    ...
    expect(sorted[sorted.length - 1].name).to.equal(
      "West London Court no hearings"
    );
  });
