...
describe.skip('Cookie policy', () => {
  ...
  it('Test updating of the datalayer for when user clicks save', async () => {
    ...
    window.dtrum = { enable: mockEnable, enableSessionReplay: mockEnableSessionReplay};
    window.dataLayer = [];
    ...
    const acceptButtonAction = () => document.getElementsByClassName('cookie-banner-accept-button')[0] as HTMLDivElement;
    ...
    expect(defaultSetting['event']).toEqual('Cookie Preferences');
    expect(defaultSetting['cookiePreferences']).toEqual({'analytics': 'on', 'apm': 'on'});
