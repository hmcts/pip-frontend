...
describe("Navigation banner tests on the view-option page", () => {
  describe("Non signed in user", () => {
    ...
    it("should display navigation banner with 4 links", () => {
      const navigationLinks = htmlRes.getElementsByClassName(
    ...
    it("links should have correct names and paths", () => {
      ...
      linkKeys.forEach((value, index) => {
        const link = navigationLinks[index].getElementsByClassName(
          "moj-sub-navigation__link"
        )[0];
        expect(link.innerHTML).contain(value, "link has incorrect name");
        expect(link.getAttribute("HREF")).contain(
          NON_SIGNED_IN_LINKS[value],
          "link has incorrect path"
        );
      });
    });
  });
  ...
  describe("Signed in user", () => {
    ...
    it("links should have correct names and paths", () => {
      ...
      const signedInLinks = htmlRes.getElementsByClassName(
        "moj-sub-navigation__item"
      );
      signedInKeys.forEach((value, index) => {
        const link = signedInLinks[index].getElementsByClassName(
          "moj-sub-navigation__link"
        )[0];
        expect(link.innerHTML).contain(value, "link has incorrect name");
        expect(link.getAttribute("HREF")).contain(
          SIGNED_IN_LINKS[value],
          "link has incorrect path"
        );
      });
    });
