...
import RemoveListConfirmationController from '../../../../main/controllers/admin/RemoveListConfirmationController';
...
describe('Remove List Confirmation Controller', () => {
    it('should render remove list confirmation page', async () => {
        ...
        responseMock.expects('render').once().withArgs('admin/remove-list-confirmation', expectedOptions);
    ...
    it('should render error page if URL', async () => {
        const request = mockRequest(i18n);
        const responseMock = sinon.mock(response);

        request.body = {
            artefactIds: [validArtefactId, validArtefactId2, validArtefactId3],
            locationId: 'http://localhost',
        };
