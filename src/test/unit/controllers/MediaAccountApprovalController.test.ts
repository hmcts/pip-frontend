...
const mediaAccountCreationStub = sinon.stub(
  MediaAccountApplicationService.prototype,
...
describe("Media Account Approval Controller", () => {
  ...
  it("should render media-account-approval page when applicant ID sent and found", async () => {
    ...
    responseMock
      ...
      .withArgs("media-account-approval", {
        ...cloneDeep(
          request.i18n.getDataByLanguage(request.lng)["media-account-approval"]
        ),
  ...
  it("should render media-account-approval-confirmation page if applicant found, approved, and successfully updated", async () => {
    ...
    mediaAccountApplicationStub
      .withArgs(applicantId, status)
      .resolves(dummyApplication);
    mediaAccountCreationStub
      .withArgs(applicantId, adminAccountId)
      .resolves(true);
