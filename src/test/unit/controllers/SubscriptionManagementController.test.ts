...
describe("Subscriptions Management Controller", () => {
  sinon
    .stub(SubscriptionService.prototype, "generateCaseTableRows")
    .returns({ cases: [] });
  sinon
    .stub(SubscriptionService.prototype, "generateLocationTableRows")
    .returns({ courts: [] });
  ...
  it("should render the subscription management page with all as default", () => {
    ...
    responseMock
      .expects("render")
      .once()
      .withArgs("subscription-management", expectedData);
  ...
  it("should render error page if there is no user data", () => {
    ...
    responseMock
      ...
      .withArgs(
        "error",
        erroredRequest.i18n.getDataByLanguage(request.lng).error
      );
