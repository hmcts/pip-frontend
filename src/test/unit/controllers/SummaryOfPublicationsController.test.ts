...
const rawSJPData = fs.readFileSync(
  path.resolve(__dirname, "../mocks/trimmedSJPCases.json"),
  "utf-8"
);
const sjpCases = JSON.parse(rawSJPData).results;
const CourtStub = sinon.stub(LocationService.prototype, "getLocationById");
const SoPStub = sinon.stub(
  SummaryOfPublicationsService.prototype,
  "getPublications"
);

describe("Get publications", () => {
  CourtStub.withArgs(9).resolves(
    JSON.parse('{"name":"Single Justice Procedure"}')
  );
  ...
  it("should render the Summary of Publications page", async () => {
    ...
    responseMock
      .expects("render")
      .once()
      .withArgs("summary-of-publications", expectedData);
