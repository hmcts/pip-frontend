...
const uploadStub = sinon.stub(
  ManualUploadService.prototype,
  "uploadPublication"
);
sinon
  .stub(ManualUploadService.prototype, "formatPublicationDates")
  .returns(mockData);
...
const removeFileStub = sinon
  .stub(FileHandlingService.prototype, "removeFileFromRedis")
  .resolves("");
removeFileStub.resolves("");

uploadStub
  .withArgs({ ...mockData, file: "", userId: "1" }, true)
  .resolves(false);
...
describe("Manual upload summary controller", () => {
  ...
  describe("GET view", () => {
    it("should render manual upload summary page", async () => {
      ...
      const options = {
        ...cloneDeep(
          request.i18n.getDataByLanguage(request.lng)["file-upload-summary"]
        ),
      ...
      responseMock
        .expects("render")
        .once()
        .withArgs("file-upload-summary", options);
  ...
  describe("POST view", () => {
    ...
    it("should redirect to success page", async () => {
      ...
      uploadStub
        .withArgs({ ...mockData, file: "", userEmail: "2" }, true)
        .resolves(res);
