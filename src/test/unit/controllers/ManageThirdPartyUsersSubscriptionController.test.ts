...
const manageThirdPartyUsersSubscriptionsController =
  new ManageThirdPartyUsersSubscriptionsController();
...
describe("Manage third party users subscription controller", () => {
  ...
  const getThirdPartyUserByIdStub = sinon.stub(
  ...
  describe("get third party subscription controller", () => {
    ...
    const getListTypesStub = sinon.stub(
      PublicationService.prototype,
      "getListTypes"
    );
    const getSubscriptionsByUserStub = sinon.stub(
      SubscriptionService.prototype,
      "getSubscriptionsByUser"
    );
    const getChannelsListStub = sinon.stub(
      SubscriptionService.prototype,
      "retrieveChannels"
    );
    const generateListTypesStub = sinon.stub(
      ThirdPartyService.prototype,
      "generateListTypes"
    );
    ...
    it("should render third party subscriptions page", async () => {
      ...
      getSubscriptionsByUserStub
        .withArgs(userId)
        .resolves({ listTypeSubscriptions: [] });
      getChannelsListStub.resolves(["CHANNEL_A", "EMAIL"]);
      generateListTypesStub
        .withArgs(["LIST_A", "LIST_B"], { listTypeSubscriptions: [] })
        .returns({});
      generateAvailableChannelsStub
        .withArgs(["CHANNEL_A"], { listTypeSubscriptions: [] })
        .returns({});
      ...
      responseMock
        .expects("render")
        .once()
        .withArgs("manage-third-party-users-subscriptions", options);
    ...
    it("should error page if user Id not present", async () => {
      ...
      responseMock
        ...
        .withArgs(
          "error",
          request.i18n.getDataByLanguage(request.lng)["error"]
        );
  ...
  describe("post third party subscription controller", () => {
    ...
    it("should render the third party confirmation page", async () => {
      ...
      getThirdPartyUserByIdStub.withArgs(userId).resolves({ userId: userId });
      updateThirdPartySubsStub
        .withArgs(userId, "CHANNEL_A", ["LIST_SELECTION"])
        .resolves();
      ...
      await manageThirdPartyUsersSubscriptionsController.post(
        request,
        response
      );
      responseMock.verify();
