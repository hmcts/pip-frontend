...
const mediaAccountRejectionStub = sinon.stub(
  MediaAccountApplicationService.prototype,
  "rejectApplication"
);
const i18n = {
...
describe("Media Account Rejection Controller", () => {
  ...
  it("should render media-account-rejection page when applicant ID sent and found", async () => {
    ...
    responseMock
      ...
      .withArgs("media-account-rejection", {
        ...cloneDeep(
          request.i18n.getDataByLanguage(request.lng)["media-account-rejection"]
        ),
  ...
  it("should render media-account-rejection-confirmation page if applicant found, approved, and successfully updated", async () => {
    ...
    mediaAccountApplicationStub
      .withArgs(applicantId, status)
      .resolves(dummyApplication);
    mediaAccountRejectionStub
      .withArgs(applicantId, adminUserId)
      .resolves(dummyApplication);
    responseMock
      .expects("redirect")
      .once()
      .withArgs(
        "/media-account-rejection-confirmation?applicantId=" + applicantId
      );
