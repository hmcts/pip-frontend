...
describe('List Download Disclaimer Controller', () => {
  ...
  jest.mock('fs');
  const generateFilesStub = sinon.stub(ListDownloadService.prototype, 'generateFiles');
  ...
  it('should set response headers when downloading Excel spreadsheet', () => {
    ...
    responseMock.expects('setHeader').once().withArgs('Content-type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
