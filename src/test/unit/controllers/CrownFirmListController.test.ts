...
const fullyProcessedData = fs.readFileSync(
  path.resolve(__dirname, "../mocks/firmlistfullyprocessed.json"),
  "utf-8"
);
const listData = JSON.parse(fullyProcessedData);

const unprocessed = fs.readFileSync(
  path.resolve(__dirname, "../mocks/crownFirmList.json"),
  "utf-8"
);
...
const crownFirmListJsonStub = sinon.stub(
  PublicationService.prototype,
  "getIndividualPublicationJson"
);
const crownFirmListMetaDataStub = sinon.stub(
  PublicationService.prototype,
...
sinon
  ...
  .returns(listData);
sinon
  .stub(CrimeListsService.prototype, "manipulatedCrimeListData")
  .returns(listData);
sinon
  .stub(CrimeListsService.prototype, "findUnallocatedCasesInCrownDailyListData")
  .returns(listData);
sinon
  .stub(CrownFirmListService.prototype, "splitOutFirmListData")
  .returns(listData);
...
describe("Crown Firm List Controller", () => {
  ...
  it("should render the crown firm list page", async () => {
    ...
    responseMock
      .expects("render")
      .once()
      .withArgs("crown-firm-list", expectedData);
