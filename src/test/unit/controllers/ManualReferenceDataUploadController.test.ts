...
const manualReferenceDataUploadController =
  new ManualReferenceDataUploadController();
describe("Reference Data Manual Upload Controller", () => {
  ...
  describe("GET", () => {
    ...
    it("should render the manual-reference-data-upload page", async () => {
      ...
      responseMock
        .expects("render")
        .once()
        .withArgs("manual-reference-data-upload", expectedData);
  ...
  describe("POST", () => {
    const fileValidationStub = sinon.stub(
      FileHandlingService.prototype,
      "validateFileUpload"
    );
    const sanatliseFileNameStub = sinon.stub(
      FileHandlingService.prototype,
      "sanitiseFileName"
    );

    sinon
      .stub(ManualUploadService.prototype, "appendlocationId")
      .resolves({ courtName: "name", id: "1" });
    ...
    it("should redirect page if no errors present", async () => {
      const fileUploadStub = sinon.stub(
        FileHandlingService.prototype,
        "storeFileIntoRedis"
      );
      ...
      responseMock
        .expects("redirect")
        .once()
        .withArgs("/manual-reference-data-upload-summary?check=true");
