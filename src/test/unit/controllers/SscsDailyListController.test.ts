...
const rawData = fs.readFileSync(
  path.resolve(__dirname, "../mocks/sscsDailyList.json"),
  "utf-8"
);
...
const sscDailyListAdditionalHearingsUrl =
...
const sscsDailyListJsonStub = sinon.stub(
  PublicationService.prototype,
  "getIndividualPublicationJson"
);
const sscsDailyListMetaDataStub = sinon.stub(
  PublicationService.prototype,
  "getIndividualPublicationMetadata"
);
sinon.stub(LocationService.prototype, "getLocationById").resolves(courtData[0]);
sinon
  .stub(SscsDailyListService.prototype, "manipulateSscsDailyListData")
  .returns(listData);

sscsDailyListJsonStub
  .withArgs(artefactIdMap.get(sscDailyListUrl), userId)
  .resolves(listData);
sscsDailyListJsonStub
  ...
  (url) => {
    it("should render the sscs daily list page", async () => {
      ...
      const expectedData = {
        ...
        ...i18n["list-template"],
        listData,
        contentDate: DateTime.fromISO(contentDate, { zone: "utc" }).toFormat(
          "dd MMMM yyyy"
        ),
      ...
      responseMock
        .expects("render")
        .once()
        .withArgs("sscs-daily-list", expectedData);
