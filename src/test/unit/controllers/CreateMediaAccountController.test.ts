...
const validateFormFieldsStub = sinon.stub(
  CreateAccountService.prototype,
  "validateFormFields"
);
validateFormFieldsStub.withArgs(validBody, validFile).returns(responseNoErrors);
validateFormFieldsStub
  .withArgs(invalidBody, invalidFileType)
  .returns(responseErrors);

const createMediaAccountStub = sinon.stub(
  CreateAccountService.prototype,
    ...
    it("should render create media account page", async () => {
      ...
      responseMock
        ...
        .withArgs(
          ...
          request.i18n.getDataByLanguage(request.lng)["create-media-account"]
        );
  ...
  describe("post requests", () => {
    it("should render create media account page", async () => {
      ...
      responseMock
        .expects("render")
        .once()
        .withArgs("create-media-account", expectedOptions);
    ...
    it("should redirect to the request submitted page if successful", async () => {
      ...
      responseMock
        .expects("redirect")
        .once()
        .withArgs("account-request-submitted");
      await createMediaAccountController.post(request, response);
      await responseMock.verify();
    });
  });
