...
describe("Test language file parser helper", () => {
  ...
  it("check method can read node from the English language file", () => {
    ...
    expect(languageFileParser.getText(fileJson, null, "nameLabel")).to.equal(
      "Full name"
    );
  });
  ...
  it("check method can read error node from the English language file", () => {
    ...
    expect(
      languageFileParser.getText(fileJson, "fullNameErrors", "blank")
    ).to.equal("There is a problem - Full name field must be populated");
  });
  ...
  it("check method can read error node from Welsh English language file", () => {
    ...
    expect(
      languageFileParser.getText(fileJson, "fullNameErrors", "blank")
    ).to.equal("Mae yna broblem - Rhaid rhoi enw llawn");
  });
});
