...
export class PendingSubscriptionsFromCache {
  public async setPendingSubscriptions(
    ...
    if (redisClient.status === "ready") {
      ...
      const rawData = await redisClient.get(
      ...
      subscriptions.forEach((subscription) => {
        ...
        if (
          !subscriptionsSet.some(
            (cached) => cached[filter] === subscription[filter]
          )
        ) {
  ...
  public async getPendingSubscriptions(userId, type): Promise<any[]> {
    ...
    if (redisClient.status === "ready" && userId) {
      cacheResult = JSON.parse(
        await redisClient.get(`pending-${type}-subscriptions-${userId}`)
      );
    }
    return cacheResult;
  }
    ...
    if (redisClient.status === "ready" && userId) {
      if (removeObject.case) {
        ...
        const filteredCases = cachedCases.filter(
          (c) => c.caseNumber !== removeObject.case
        );
        redisClient.set(
          `pending-cases-subscriptions-${userId}`,
      ...
      if (removeObject.court || removeObject.court === 0) {
        const cachedCourts = await this.getPendingSubscriptions(
          userId,
          "courts"
        );
        const filteredCourts = cachedCourts.filter(
          (court) => court.locationId !== parseInt(removeObject.court)
        );
        redisClient.set(
          `pending-courts-subscriptions-${userId}`,
          JSON.stringify(filteredCourts)
        );
      }
    }
  }
