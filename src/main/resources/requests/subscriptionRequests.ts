...
export class SubscriptionRequests {
  public async getUserSubscriptions(
    ...
    try {
      const response = await subscriptionManagementApi.get(
  ...
  public async unsubscribe(
    ...
    try {
      const response = await subscriptionManagementApi.delete(
  ...
  public async configureListTypeForLocationSubscriptions(
    ...
    try {
      await subscriptionManagementApi.put(
        `/subscription/configure-list-types/${userId}`,
        payload
      );
      return true;
    } catch (error) {
      if (error.response) {
        ...
        console.log(
          "Unknown error while configuring list type for location subscription"
        );
      }
    }
    return false;
  ...
  public async retrieveSubscriptionChannels(): Promise<string[]> {
    try {
      const channelResponse = await subscriptionManagementApi.get(
        "/meta/channels"
      );
      ...
      if (error.response) {
        ...
        console.log(
          "Unknown error while attempting to retrieve the list of channels"
        );
      }
    }
    return [];
