...
function createFilters(env) {
  ...
  const languageLookup = JSON.parse(
    fs.readFileSync(path.resolve(__dirname, "languageLookup.json"))
  );
  ...
  env.addFilter("dateRange", function (x) {
    return (
      DateTime.fromISO(x.displayFrom, { zone: "Europe/London" }).toFormat(
        "dd MMM yyyy"
      ) +
      " to " +
      DateTime.fromISO(x.displayTo, { zone: "Europe/London" }).toFormat(
        "dd MMM yyyy"
      )
    );
  });
  ...
  env.addFilter("emailLink", function (x) {
    return this.env.filters.safe(
      '<a class=govuk-link href="mailto:' + x + '">' + x + "</a>"
    );
  });
  ...
  env.addFilter("phoneLink", function (x) {
    return this.env.filters.safe(
      '<a class=govuk-link href="tel:' + x + '">' + x + "</a>"
    );
  });
  ...
  env.addFilter("getDuration", function (hours, mins, language) {
    return [
      printableDuration(hours, "hour", language),
      printableDuration(mins, "min", language),
    ]
      .join(" ")
      .trim();
  });
