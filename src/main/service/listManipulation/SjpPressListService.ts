            ...
            sitting["hearing"].forEach((hearing) => {
              ...
              hearing["party"].forEach((party) => {
                if (party["individualDetails"]) {
                  party["individualDetails"]["formattedDateOfBirth"] =
                    DateTime.fromISO(
                      party["individualDetails"]["dateOfBirth"]
                        .split("/")
                        .reverse()
                        .join("-")
              ...
              hearing["offence"].forEach((offence) => {
                const reportingRestriction =
                  offence["reportingRestriction"].toString();
                offence["formattedReportingRestriction"] =
                  reportingRestriction.charAt(0).toUpperCase() +
