...
export class MagistratesStandardListService {
  ...
  private combineDefendantSittings(allDefendants): object {
    ...
    const uniqueDefendantNames = helperService.uniquesInArrayByAttrib(
      allDefendants,
      "defendantHeading"
    );
    uniqueDefendantNames.forEach((defendantNames) => {
      const defendant = allDefendants.filter(
        (row) => row.defendantHeading === defendantNames
      );
  ...
  private manipulateHearingObject(hearingObject, party, language) {
    ...
    hearingObject["case"].forEach((thisCase) => {
      hearingObject["formattedConvictionDate"] = formatDate(
        thisCase["convictionDate"],
        "dd/MM/yyyy",
        language
      );
      hearingObject["formattedAdjournedDate"] = formatDate(
        thisCase["adjournedDate"],
        "dd/MM/yyyy",
        language
      );
      hearingObject["caseSequenceIndicator"] =
        thisCase["caseSequenceIndicator"];
      hearingObject["hearingNumber"] = thisCase["hearingNumber"];
      hearingObject["prosecutionAuthorityCode"] =
        thisCase["informant"]["prosecutionAuthorityCode"];
  ...
  private formatDefendant(
    ...
    return {
      defendantHeading: this.formatDefendantHeading(
        defendantHeading,
        gender,
        inCustody
      ),
  ...
  private manipulatePartyInformation(hearing: any, party: any): void {
    ...
    if (
      ListParseHelperService.convertPartyRole(party.partyRole) === "DEFENDANT"
    ) {
      defendant = crimeListsService
        .createIndividualDetails(party.individualDetails)
        .trim();
    }

    hearing["defendantHeading"] = defendant?.replace(/,\s*$/, "").trim();
    hearing["defendantDateOfBirth"] = party?.individualDetails?.dateOfBirth;
    hearing["defendantAddress"] = this.formatDefendantAddress(
      party.individualDetails?.address
    );
    ...
    if (party?.individualDetails?.inCustody) {
      hearing["inCustody"] =
        party.individualDetails?.inCustody === true ? "*" : "";
    }
  }
  ...
  private formatDefendantAddress(defendantAddress: object): string {
    ...
    if (defendantAddress !== null) {
      ...
      formattedAddress += defendantAddress["town"]
        ? ", " + defendantAddress["town"]
        : "";
      formattedAddress += defendantAddress["county"]
        ? ", " + defendantAddress["county"]
        : "";
      formattedAddress += defendantAddress["postCode"]
