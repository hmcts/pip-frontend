...
export class EtListsService {
    ...
    jsonData["courtLists"].forEach((courtList) => {
      courtList["courtHouse"]["courtRoom"].forEach((courtRoom) => {
        courtRoom["session"].forEach((session) => {
          session["formattedJudiciary"] =
            this.helperService.getJudiciaryNameSurname(session);
          delete session["judiciary"];
          session["sittings"].forEach((sitting) => {
            sitting["sittingStartFormatted"] = formatDate(
              sitting["sittingStart"],
              "h:mma",
              language
            );
            this.helperService.calculateDuration(sitting);
            this.helperService.findAndConcatenateHearingPlatform(
              sitting,
              session
            );
            sitting["hearing"].forEach((hearing) => {
              this.helperService.findAndManipulatePartyInformation(
                hearing,
                true
              );
            });
   ...
   */
  public reshapeEtFortnightlyListData(
    etFortList: string,
    language: string
  ): object {
    return this.dataSplitterEtList(
      this.reshapeEtLists(etFortList, language),
      language
    );
  }
            ...
            sitting["hearing"].forEach((hearing) => {
              hearing["case"].forEach((thisCase) => {
                const row = {
                  ...
                  sittingTime: formatDate(
                    sitting["sittingStart"],
                    "h:mma",
                    language
                  ),
                  addressLine:
                    courtList["courtHouse"]["courtHouseAddress"]["line"],
                  addressTown:
                    courtList["courtHouse"]["courtHouseAddress"]["town"],
                  addressCounty:
                    courtList["courtHouse"]["courtHouseAddress"]["county"],
                  addressPostCode:
                    courtList["courtHouse"]["courtHouseAddress"]["postCode"],
  ...
  private splitByCourtAndDate(data: any) {
    const courts = [];
    const uniqueCourts = this.helperService.uniquesInArrayByAttrib(
      data,
      "courtName"
    );
    let courtCounter = 0;
    uniqueCourts.forEach((court) => {
      ...
      const uniqueDays = this.helperService.uniquesInArrayByAttrib(
        courtData,
        "sittingDate"
      );
      const uniqueDaysArr = [];
