...
export class CrimeListsService {
  public manipulatedCrimeListData(
    crimeListData: string,
    language: string,
    languageFile: string
  ): object {
    ...
    if (hearing?.party) {
      hearing.party.forEach((party) => {
        switch (party.partyRole) {
          case "DEFENDANT": {
            const defendant = this.createIndividualDetails(
          ...
          case "DEFENDANT_REPRESENTATIVE": {
            const defendantRepresentative = this.createOrganisationDetails(
          ...
          case "PROSECUTING_AUTHORITY": {
            const prosecutingAuthority = this.createOrganisationDetails(
              party.organisationDetails
            );
      ...
      hearing.defendantRepresentative =
        defendantRepresentatives.join(separator);
  ...
  private createOrganisationDetails(organisationDetails: any) {
    return ListParseHelperService.writeStringIfValid(
      ...
      hearing.case.forEach((cases) => {
        ...
        if (cases?.caseLinked) {
          cases.caseLinked.forEach((caseLinked) => {
            ...
            linkedCases += helperService.stringDelimiter(
              linkedCases?.length,
              ","
            );
          });
  ...
  public findUnallocatedCasesInCrownDailyListData(
    crimeListData: string
  ): Array<object> {
  ...
  private formatUnallocatedCourtList(
    ...
    unallocatedCasesCrownListData["courtLists"].push(
      courtListForUnallocatedCases
    );
  }
}
