...
export class SessionManagementService {
  ...
  private isSessionExpired(req): boolean {
    ...
    if (req.session.sessionExpires) {
      const sessionExpiryDateTime = DateTime.fromJSDate(
        req.session.sessionExpires,
        { zone: "utc" }
      );
      const currentDateTime = DateTime.fromISO(DateTime.now(), { zone: "utc" });
      const durationAsSeconds = sessionExpiryDateTime
        .diff(currentDateTime, ["seconds"])
        .as("seconds");
    ...
    const sessionExpiry = checkRoles(req, allAdminRoles)
      ? 4 * defaultSessionExpiry
  ...
  private logOutUrl(
    ...
    if (adminWrongFlow) {
      ...
      b2cPolicy = isAdmin
        ? authenticationConfig.ADMIN_POLICY
        : authenticationConfig.POLICY;
    }
  ...
  private logOutRedirectUrl(
    ...
    const url = new URL(
      `${FRONTEND_URL}/${this.getRedirectionPath(
        adminWrongFlow,
        isSessionExpired
      )}`
    );
    ...
    if (isSessionExpired) {
      url.searchParams.append(
        "reSignInUrl",
        isAdmin ? "admin-dashboard" : "sign-in"
      );
    }
    return url.toString();
  }

  private getRedirectionPath(
    adminWrongFlow: boolean,
    isSessionExpired: boolean
  ): string {
    if (adminWrongFlow) {
