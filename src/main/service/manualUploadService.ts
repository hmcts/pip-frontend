...
export class ManualUploadService {
  ...
  public getListItemName(itemValue: string): string {
    return publicationService.getListTypes().get(itemValue)
  ...
  public async validateFormFields(
    formValues: object,
    language: string,
    languageFile: string
  ): Promise<object> {
    const fields = {
      courtError: await this.validateCourt(
        formValues["input-autocomplete"],
        language,
        languageFile
      ),
  ...
  private async validateCourt(
    ...
    if (courtName?.length >= 3) {
      const validCourt = await courtService.getLocationByName(
        courtName,
        language
      );
      if (validCourt) {
  ...
  public async uploadPublication(
    data: any,
    ...
    if (fileHandlingService.getFileExtension(data.fileName) === "json") {
      return await dataManagementRequests.uploadJSONPublication(
        data,
        this.generatePublicationUploadHeaders(
          this.formatPublicationDates(data, ISODateFormat)
        )
      );
    } else {
  ...
  public formatPublicationDates(formData: any, defaultFormat: boolean): object {
    return {
      ...formData,
      "display-from": this.checkAndFormatDates(
        formData["display-from"],
        defaultFormat
      ),
      "display-to": this.checkAndFormatDates(
        formData["display-to"],
        defaultFormat
      ),
      "content-date-from": this.checkAndFormatDates(
        formData["content-date-from"],
        defaultFormat
      ),
    };
  }
  ...
  private checkAndFormatDates(date: any, defaultFormat: boolean): string {
    if (date != null) {
      return defaultFormat
        ...
        : DateTime.fromFormat(date, "dd/MM/yyyy HH:mm:ss").toFormat(
            "d MMMM yyyy"
          );
    }
    return "";
