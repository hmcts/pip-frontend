...
export class AuditLogService {
  ...
  public getTableHeaders() {
    return [{text: 'Timestamp', classes: 'govuk-!-padding-top-0'},
  ...
  public async getFormattedAuditData(pageNumber: number, adminUserId: string) {
    const rawData = await accountManagementRequests.getAllAuditLogs({params: {pageNumber: (pageNumber - 1),
      pageSize: 10}}, adminUserId);
