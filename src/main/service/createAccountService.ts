...
export class CreateAccountService {
  public validateFormFields(
    ...
    return {
      nameError: {
        message: this.validateMediaFullName(
          formValues["fullName"],
          language,
          languageFile
        ),
      ...
      emailError: {
        message: this.validateMediaEmailAddress(
      ...
      employerError: {
        message: this.validateMediaEmployer(
          formValues["employer"],
          language,
          languageFile
        ),
      ...
      fileUploadError: {
        message: fileHandlingService.validateImage(
          file,
          language,
          languageFile
        ),
   ...
   */
  public validateAdminFormFields(
    formValues: object,
    language: string,
    languageFile: string
  ): object {
   ...
   * @param languageFile The language file to use.
   */
  public validateAdminFormFieldsWithRole(
    formValues: object,
    language: string,
    languageFile: string
  ): object {
    ...
    const stateReturn = this.validateAdminFormFields(
      formValues,
      language,
      languageFile
    );
  ...
  isValidEmail(email: string): boolean {
    const emailRegex =
      /^[a-zA-Z0-9._-]{0,40}@[a-zA-Z0-9.-]{0,40}\.[a-zA-Z]{2,5}$/;
  ...
  validateEmail(email: string, language: string, languageFile: string): string {
    ...
    if (this.isNotBlank(email)) {
      if (!this.isValidEmail(email)) {
        message = languageFileParser.getText(
          fileJson,
          "emailErrors",
          "invalidEmailAddress"
        );
  ...
  validateCsvFileContent(
    ...
    if (rows.length > 0) {
      ...
      if (
        JSON.stringify(rows[0].sort()) !== JSON.stringify(expectedHeader.sort())
      ) {
        return languageFileParser.getText(
          fileJson,
          "fileUploadErrors",
   ...
   */
  public async createSystemAdminAccount(
    payload: object,
    requester: string
  ): Promise<any> {
  ...
  public async createMediaAccount(
    payload: object,
    requester: string
  ): Promise<boolean> {
    const azureResponse = await accountManagementRequests.createAzureAccount(
  ...
  public async createMediaApplication(
    payload: object,
    file: File
  ): Promise<boolean> {
    return await accountManagementRequests.createMediaAccount(
      this.formatCreateMediaAccount(payload, file)
    );
  }
  ...
  public async bulkCreateMediaAccounts(
    ...
    return await accountManagementRequests.bulkCreateMediaAccounts(
      file,
      filename,
      id
    );
