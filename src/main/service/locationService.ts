...
export class LocationService {
  ...
  public async fetchAllLocations(language: string): Promise<Array<Location>> {
    return this.initalizeLocationsForLanguage(
      await locationRequest.getAllLocations(),
      language
    );
  }

  private initalizeLocationsForLanguage(
      ...
      case "cy": {
        locations.forEach((value) => {
          const locationInfo = {
            ...
            jurisdiction:
              value["welshJurisdiction"] != null
                ? value["welshJurisdiction"]
                : value.jurisdiction,
            region:
              value["welshRegion"] != null
                ? value["welshRegion"]
                : value.region,
  ...
  public async generateAlphabetisedAllCourtList(
    language: string
  ): Promise<object> {
    return this.generateAlphabetisedCourtList(
      await this.fetchAllLocations(language)
    );
  }

  public async generateAlphabetisedCrownCourtList(
    language: string
  ): Promise<object> {
    const regions = "";
    const jurisdictions = "Crown";
    return this.generateFilteredAlphabetisedCourtList(
      regions,
      jurisdictions,
      language
    );
  ...
  private generateAlphabetisedCourtList(
    listToAlphabetise: Array<Location>
  ): object {
    ...
    sortedCourtsList.forEach((item) => {
      ...
      alphabetisedCourtList[locationName.charAt(0).toUpperCase()][
        locationName
      ] = {
        id: item.locationId,
      };
  ...
  public findCourtName(
    ...
    switch (language) {
      case "cy": {
        courtName =
          location["welshName"] != null ? location["welshName"] : location.name;
        break;
      }
