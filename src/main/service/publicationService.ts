...
export class PublicationService {
  public async getIndividualPublicationMetadata(
    artefactId,
    userId: string,
    admin = false
  ): Promise<any> {
    return publicationRequests.getIndividualPublicationMetadata(
      artefactId,
      userId,
      admin
    );
  ...
  public async getIndividualPublicationFile(
    artefactId,
    userId: string
  ): Promise<Blob> {
  ...
  public async getIndividualPublicationJson(
    artefactId,
    userId: string
  ): Promise<JSON> {
  ...
  public async getCasesByCaseName(
    ...
    const artefacts = await publicationRequests.getPublicationByCaseValue(
      "CASE_NAME",
      caseName,
      userId
    );
  ...
  public async getCaseByCaseNumber(
    caseNumber: string,
    userId: string
  ): Promise<SearchObject> | null {
    const artefact = await publicationRequests.getPublicationByCaseValue(
      "CASE_ID",
      caseNumber,
      userId
    );
  ...
  public async getPublicationsByCourt(
    locationId: string,
    userId: string
  ): Promise<Artefact[]> {
    return await publicationRequests.getPublicationsByCourt(
      locationId,
      userId,
      false
    );
  ...
  private getFuzzyCasesFromArtefact(
    artefacts: Artefact[],
    value: string
  ): SearchObject[] {
