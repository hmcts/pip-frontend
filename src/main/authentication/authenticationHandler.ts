...
export const adminAccountCreationRoles = [
  "SYSTEM_ADMIN",
...
export const mediaAccountCreationRoles = [
  "INTERNAL_SUPER_ADMIN_CTSC",
  "INTERNAL_ADMIN_CTSC",
];
...
export function checkAuthenticatedAdmin(
  req: any,
  res,
  next,
  roles: string[]
...
export function checkAuthenticatedMedia(
  req: any,
  res,
  next,
  roles: string[]
...
export async function mediaVerificationHandling(req, res): Promise<any> {
  if (req.user && verifiedRoles.includes(req.user.roles)) {
    await AccountManagementRequests.prototype.updateMediaAccountVerification(
      req.user["oid"]
    );
...
export async function processAdminAccountSignIn(req, res): Promise<any> {
  if (checkRoles(req, allAdminRoles)) {
    await AccountManagementRequests.prototype.updateAccountLastSignedInDate(
      "PI_AAD",
      req.user["oid"]
    );
