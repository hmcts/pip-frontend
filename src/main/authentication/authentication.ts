...
function oidcSetup(): void {
  ...
  if (process.env.CONFIG_ENDPOINT) {
    ...
    identityMetadata = config.get(
      "secrets.pip-ss-kv.CONFIG_ENDPOINT"
    ) as string;
  }
  ...
  if (process.env.CONFIG_ADMIN_ENDPOINT) {
    ...
    adminIdentityMetadata = config.get(
      "secrets.pip-ss-kv.CONFIG_ADMIN_ENDPOINT"
    ) as string;
  }

  if (process.env.MEDIA_VERIFICATION_CONFIG_ENDPOINT) {
    mediaVerificationIdentityMetadata =
  ...
  logger.info(
    "secret",
    clientSecret ? clientSecret.substring(0, 5) : "client secret not set!"
  );

  const piAadVerifyFunction = async function (
    iss,
    sub,
    profile,
    accessToken,
    ...
    const returnedUser = await accountManagementRequests.getPiUserByAzureOid(
      profile["oid"]
    );
  ...
  passport.serializeUser(async function (foundUser, done) {
    if (foundUser["flow"] === "CFT") {
      const user = await accountManagementRequests.getPiUserByCftID(
        foundUser["uid"]
      );
  ...
  passport.deserializeUser(async function (userDetails, done) {
    ...
    if (userDetails["flow"] === "CFT") {
      user = await accountManagementRequests.getPiUserByCftID(
        userDetails["uid"]
      );
    } else {
      user = await accountManagementRequests.getPiUserByAzureOid(
        userDetails["oid"]
      );
    }
