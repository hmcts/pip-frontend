...
export default class MediaAccountReviewController {
  public async get(req: PipRequest, res: Response): Promise<void> {
    ...
    if (applicantData) {
      return res.render("media-account-review", {
        ...cloneDeep(
          req.i18n.getDataByLanguage(req.lng)["media-account-review"]
        ),
  ...
  public async getImage(req: PipRequest, res: Response): Promise<void> {
    ...
    const applicant = await mediaAccountApplicationService.getApplicationById(
      applicantId
    );

    if (image && applicant) {
      const imageName = applicant.imageName;
      const extension = imageName.substring(
