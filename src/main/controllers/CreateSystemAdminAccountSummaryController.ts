  ...
  public async post(req: PipRequest, res: Response): Promise<void> {
    ...
    if (response) {
      if (response['error'] && !response['duplicate'] && !response['aboveMaxSystemAdmin']) {
        await userManagementService.auditAction(req.user['userId'], req.user['email'],
        ...
        await userManagementService.auditAction(req.user['userId'], req.user['email'], 'SYSTEM_ADMIN_CREATION',
