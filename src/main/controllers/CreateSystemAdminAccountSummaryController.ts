...
export default class CreateSystemAdminAccountSummaryController {
  public get(req: PipRequest, res: Response): void {
    const formData = req.cookies?.createAdminAccount
      ? JSON.parse(req.cookies["createAdminAccount"])
      : {};
  ...
  public async post(req: PipRequest, res: Response): Promise<void> {
    ...
    const response = await createAccountService.createSystemAdminAccount(
      formData,
      req.user?.["userId"]
    );
    if (response) {
      if (
        ...
        res.render("create-system-admin-account-confirm", {
          ...
          ...cloneDeep(
            req.i18n.getDataByLanguage(req.lng)[
              "create-system-admin-account-confirm"
            ]
          ),
        });
      }
