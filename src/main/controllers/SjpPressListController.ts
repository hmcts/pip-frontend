...
export default class SjpPressListController {
  public async get(req: PipRequest, res: Response): Promise<void> {
    ...
    const sjpData = await publicationService.getIndividualPublicationJson(
      artefactId,
      req.user?.["userId"]
    );
    ...
    if (sjpData && metaData) {
      const manipulatedData = sjpPressListService.formatSJPPressList(
        JSON.stringify(sjpData)
      );
      ...
      res.render("single-justice-procedure-press", {
        ...cloneDeep(
          req.i18n.getDataByLanguage(pageLanguage)[
