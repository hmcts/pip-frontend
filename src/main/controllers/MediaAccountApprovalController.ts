...
export default class MediaAccountApprovalController {
  public async get(req: PipRequest, res: Response): Promise<void> {
    ...
    const applicantData =
      await mediaAccountApplicationService.getApplicationByIdAndStatus(
        applicantId,
        "PENDING"
      );
    ...
    if (applicantData) {
      res.render("media-account-approval", {
        ...cloneDeep(
          req.i18n.getDataByLanguage(req.lng)["media-account-approval"]
        ),
   ...
   */
  private static applicationFoundFlow(
    req,
    res,
    approved,
    applicantId,
    ...
    if (approved === "Yes") {
      return MediaAccountApprovalController.approvalFlow(
        req,
        res,
        applicantId,
        applicantData
   ...
   */
  private static async approvalFlow(
    req,
    res,
    applicantId,
    applicantData
    ...
    if (
      await mediaAccountApplicationService.createAccountFromApplication(
        applicantId,
        req.user?.["userId"]
      )
    ) {
      return res.redirect(
        "/media-account-approval-confirmation?applicantId=" + applicantId
      );
    } else {
      return res.render("media-account-approval", {
        ...cloneDeep(
