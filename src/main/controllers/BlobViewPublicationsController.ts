...
export default class BlobViewPublicationsController {
  public async get(req: PipRequest, res: Response): Promise<void> {
    ...
    if (locationId) {
      const court = await locationService.getLocationById(
      ...
      const locationName = locationService.findCourtName(
        court,
        req.lng as string,
        "summary-of-publications"
      );
      const listOfPublications =
        await summaryOfPublicationsService.getPublications(
          parseInt(locationId.toString()),
          req.user?.["userId"]
        );
      ...
      res.render("blob-view-publications", {
        ...cloneDeep(
          req.i18n.getDataByLanguage(req.lng)["blob-view-publications"]
        ),
