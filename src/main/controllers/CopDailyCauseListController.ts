...
export default class CopDailyCauseListController {
  public async get(req: PipRequest, res: Response): Promise<void> {
    ...
    if (searchResults && metaData) {
      const manipulatedData = copDailyListService.manipulateCopDailyCauseList(
      ...
      const regionalJoh = helperService.getRegionalJohFromLocationDetails(
        searchResults["locationDetails"]
      );

      res.render("cop-daily-cause-list", {
        ...cloneDeep(
          req.i18n.getDataByLanguage(pageLanguage)["cop-daily-cause-list"]
        ),
