...
export default class CreateMediaAccountController {
  public get(req: PipRequest, res: Response): void {
    res.render(
      ...
      req.i18n.getDataByLanguage(req.lng)["create-media-account"]
    );
  }
  ...
  public async post(req: PipRequest, res: Response): Promise<void> {
    ...
    const isValidForm = Object.values(formValidation).every(
      (o) => o.message === null
    );

    if (isValidForm) {
      const response = await createAccountService.createMediaApplication(
        req.body,
        req.file
      );
