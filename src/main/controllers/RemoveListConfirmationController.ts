...
export default class RemoveListConfirmationController {
  public async get(req: PipRequest, res: Response): Promise<void> {
    ...
    if (artefactId) {
      const artefact =
        await publicationService.getIndividualPublicationMetadata(
          artefactId,
          req.user?.["userId"],
          true
        );
      artefact.listTypeName = manualUploadService.getListItemName(
        artefact.listType
      );
      ...
      res.render("remove-list-confirmation", {
        ...cloneDeep(
          req.i18n.getDataByLanguage(req.lng)["remove-list-confirmation"]
        ),
        artefact,
  ...
  public async post(req: PipRequest, res: Response): Promise<void> {
    ...
    switch (formData["remove-choice"]) {
      case "yes": {
        const response = await publicationService.removePublication(
          formData.artefactId,
          req.user?.["userId"]
        );
        response
      ...
      case "no": {
        res.redirect(
          `/remove-list-search-results?locationId=${formData.locationId}`
        );
        break;
      }
      default:
