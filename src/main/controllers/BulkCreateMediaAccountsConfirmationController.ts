...
const bulkCreateAccountsConfirmationUrl =
...
export default class BulkCreateMediaAccountsConfirmationController {
  public async get(req: PipRequest, res: Response): Promise<void> {
    const formData = req.cookies?.formCookie
      ? JSON.parse(req.cookies["formCookie"])
      : {};
    ...
    const accountsToCreate =
      await BulkCreateMediaAccountsConfirmationController.getAccountsToCreate(
        req.user["userId"],
        fileName
      );
    ...
    res.render(bulkCreateAccountsConfirmationUrl, {
      ...cloneDeep(
        req.i18n.getDataByLanguage(req.lng)[bulkCreateAccountsConfirmationUrl]
      ),
      accountsToCreate,
    });
  }
  ...
  public async post(req: PipRequest, res: Response): Promise<void> {
    ...
    if (confirmed === "Yes") {
      ...
      const success = await createAccountService.bulkCreateMediaAccounts(
        file,
        fileName,
        req.user?.["userId"]
      );
      if (success) {
        ...
        return res.render(bulkCreateAccountsConfirmationUrl, {
          ...cloneDeep(
            req.i18n.getDataByLanguage(req.lng)[
              bulkCreateAccountsConfirmationUrl
            ]
          ),
          accountsToCreate,
  ...
  private static async getAccountsToCreate(
    userId,
    fileName
  ): Promise<string[][]> {
    ...
    return BulkCreateMediaAccountsConfirmationController.reorderFieldsInRows(
      fileData
    );
  }
  ...
  private static reorderFieldsInRows(rows): string[][] {
    ...
    rows.slice(1).forEach((row) => {
      const fields = [emailIndex, firstNameIndex, surnameIndex].map(
        (i) => row[i]
      );
