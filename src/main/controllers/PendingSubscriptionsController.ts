...
export default class PendingSubscriptionsController {
  public async get(req: PipRequest, res: Response): Promise<void> {
    const pendingSubscriptions = {
      cases: await subscriptionService.getPendingSubscriptions(
        req.user["userId"],
        "cases"
      ),
      courts: await subscriptionService.getPendingSubscriptions(
        req.user["userId"],
        "courts"
      ),
    };
    ...
    req.query?.["no-subscriptions"]
      ? res.render("pending-subscriptions", {
          ...cloneDeep(
            req.i18n.getDataByLanguage(req.lng)["pending-subscriptions"]
          ),
