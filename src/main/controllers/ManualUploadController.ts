  ...
  public async post(req: PipRequest, res: Response): Promise<void> {
    if (req.query?.showerror === "true") {
      ...
      const errors = {
        ...
        formErrors: await manualUploadService.validateFormFields(
          req.body,
          req.lng as string,
          "manual-upload"
        ),
      ...
      if (errors.fileErrors || errors.formErrors) {
        ...
        const sanitisedFileName =
          fileHandlingService.sanitiseFileName(originalFileName);
        ...
        req.body["display-from"] = manualUploadService.buildDate(
          req.body,
          "display-date-from"
        );
        req.body["display-to"] = manualUploadService.buildDate(
          req.body,
          "display-date-to"
        );
        req.body["content-date-from"] = manualUploadService.buildDate(
          req.body,
          "content-date-from"
        );
        ...
        if (req.body?.classification) {
          req.body["classificationName"] = formValues[
            "form"
          ].classification.find(
