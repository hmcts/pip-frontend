...
export default class CrownWarnedListController {
  public async get(req: PipRequest, res: Response): Promise<void> {
    ...
    if (searchResults && metaData) {
      ...
      const listData = crownWarnedListService.manipulateData(
      ...
      const sortedListData = new Map(
        [...listData].sort(([a], [b]) =>
          toBeAllocatedLowerCase === a.toLowerCase()
            ? 1
            : toBeAllocatedLowerCase === b.toLowerCase()
            ? -1
            : 0
        )
      );
      ...
      res.render(listUrl, {
        ...
        contentDate: crownWarnedListService.formatContentDate(
