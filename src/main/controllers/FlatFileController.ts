...
export default class FlatFileController {
  public async get(req: PipRequest, res: Response): Promise<void> {
    ...
    const fileData = await publicationService.getIndividualPublicationFile(
      artefactId,
      req.user?.["userId"]
    );
    res.set(
      "Content-Disposition",
      "inline;filename=" + metadata.sourceArtefactId
    );
    if (metadata.sourceArtefactId.endsWith(".pdf")) {
      ...
      res.set(
        "Content-Disposition",
        "attachment;filename=" + metadata.sourceArtefactId
      );
    }
    res.send(fileData);
  }
