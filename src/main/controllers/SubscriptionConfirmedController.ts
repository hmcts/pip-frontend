...
export default class SubscriptionConfirmedController {
  public async post(req: PipRequest, res: Response): Promise<void> {
    ...
    const cachedCourts = await cacheService.getPendingSubscriptions(
      userId,
      "courts"
    );
    const cachedCases = await cacheService.getPendingSubscriptions(
      userId,
      "cases"
    );
    ...
    if (cachedCases?.length || cachedCourts?.length) {
      ...
      subscribed
        ? res.render(
            ...
            req.i18n.getDataByLanguage(req.lng)["subscription-confirmed"]
          )
