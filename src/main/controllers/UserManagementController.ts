...
export default class UserManagementController {
  public async get(req: PipRequest, res: Response): Promise<void> {
    ...
    if (req.query.clear) {
      const responseBody = userManagementService.handleFilterClearing(
        req.query
      );
      const filterValues =
        userManagementService.generateFilterKeyValues(responseBody);
  ...
  public async post(req: PipRequest, res: Response): Promise<void> {
    const filterValues = userManagementService.generateFilterKeyValues(
      req.body
    );
    res.redirect(`user-management?${filterValues}`);
  }
