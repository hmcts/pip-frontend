...
export default class BulkUnsubscribeController {
  ...
  public async post(req: PipRequest, res: Response): Promise<void> {
    const subscriptionsToDelete =
      BulkUnsubscribeController.getSelectedSubscriptions(req.body);
    if (req.user) {
      if (subscriptionsToDelete.length == 0) {
        ...
        res.render(bulkUnsubscribeConfirmationUrl, {
          ...cloneDeep(
            req.i18n.getDataByLanguage(req.lng)[bulkUnsubscribeConfirmationUrl]
          ),
  ...
  private static getSelectedSubscriptions(body): string[] {
    ...
    if (caseSubscription !== undefined) {
      BulkUnsubscribeController.addToSubscriptionsForDeletion(
        caseSubscription,
  ...
  private static addToSubscriptionsForDeletion(
    subscription,
    subscriptionsToDelete
  ): void {
