...
export default class LocationDataManualUploadSummaryController {
  public get(req: PipRequest, res: Response): void {
    const formData = (req.cookies?.formCookie) ? JSON.parse(req.cookies['formCookie']) : {};
    (req.query?.error === 'true') ?
      res.render('location-data-manual-upload-summary', {
        ...cloneDeep(req.i18n.getDataByLanguage(req.lng)['location-data-manual-upload-summary']),
        displayError: true,
  ...
  public async post(req: PipRequest, res: Response): Promise<void> {
    ...
    if (req.query?.check === 'true') {
      ...
      if (response) {
        ...
        res.redirect('location-data-upload-confirmation');
      } else {
        res.render('location-data-manual-upload-summary', {
          ...cloneDeep(req.i18n.getDataByLanguage(req.lng)['location-data-manual-upload-summary']),
          fileUploadData: formData,
